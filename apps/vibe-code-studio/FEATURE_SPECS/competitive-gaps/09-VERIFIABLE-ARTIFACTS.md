...
**Status**: 📋 PLANNED (PARTIAL — `TaskMonitorPanel` shows raw `BackgroundTask` progress/logs, but there is no first-class Artifact model, no artifact panel, and no non-blocking inline commenting)
**Priority**: HIGH
...
1. ⬜ An `Artifact` type is defined with `id`, `taskId`, `kind` (`task_list | plan | walkthrough | screenshot | diff`), `title`, `content` (markdown or structured JSON depending on kind), `createdAt`, `status` (`draft | final`).
2. ⬜ Artifacts are persisted via `DatabaseService` in a new `artifacts` table, queryable by `taskId` and `kind`.
3. ⬜ An **Artifacts panel** (new sidebar/tab, sibling to `TaskMonitorPanel`) lists artifacts grouped by task, with type icons and a preview.
4. ⬜ Clicking a `diff` artifact opens it in the existing `MultiFileDiffView.tsx` rather than a new diff renderer.
5. ⬜ Clicking a `plan` artifact opens the same markdown+Mermaid viewer built for spec 05's plan files (reuses `.vcs/plans/*.md` when the artifact wraps a plan file).
6. ⬜ Inline comments can be added to any artifact at a specific location (line for diff/plan, region for screenshot); comments are stored with `artifactId`, `anchor`, `body`, `resolved`.
7. ⬜ Submitting a comment while the originating agent task is still running pushes a structured message onto that task's work queue **without pausing or cancelling** the running task.
8. ⬜ A `walkthrough` artifact is auto-generated when a task reaches `completed` status, summarizing steps taken, files changed, and embedding any `screenshot` artifacts captured during the run.
9. ⬜ Every artifact list item links back to its originating `BackgroundTask` (deep link into `TaskMonitorPanel`'s existing task detail view).
10. ⬜ Artifacts persist after the workspace closes and reopens (backed by `DatabaseService`, not in-memory zustand state).
11. ⬜ Unresolved comments are visually distinguished from resolved ones in the Artifacts panel, and resolving a comment does not delete it (audit trail preserved).
12. ⬜ A `task_list` artifact is generated at task start (not just completion), so reviewers can see the intended scope even while the agent is still running.
