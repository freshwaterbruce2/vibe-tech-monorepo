...
const resolveModelId = (candidate: unknown): string => {
  if (typeof candidate === 'string' && VALID_MODEL_IDS.has(candidate)) {
    return candidate;
  }
    ...
    persist(
      ...
      })),
      {
        ...
        merge: (persistedState: any, currentState: AIState) => {
          ...
          if (persistedModel && resolvedModel !== persistedModel) {
            logger.warn(
              `[AIStore] Invalid persisted model: ${persistedModel}, falling back to ${DEFAULT_MODEL}`
            );
