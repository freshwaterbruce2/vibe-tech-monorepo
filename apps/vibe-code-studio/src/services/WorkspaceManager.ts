...
export class WorkspaceManager {
  ...
  constructor() {
    ...
    // Storage loading must be initialized explicitly via initialize()
  }
  ...
  private async loadFromStorage(): Promise<void> {
    try {
      ...
      if (current) {
        // Validate: only use if it looks like an absolute path or virtual path
        const isAbsolutePath = /^[A-Za-z]:[\\/]|^\/|^demo:\/\//.test(current);
        if (isAbsolutePath) {
          this.currentWorkspace = current;
        } else {
          // Clear invalid workspace path (relative paths cause ENOENT)
