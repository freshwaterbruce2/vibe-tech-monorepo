...
export const fileIndexingExecutor: TaskExecutor = {
  async execute(
    ...
    try {
      const workspacePath = (task.metadata?.['workspacePath'] as string) || '';
...
export const gitOperationExecutor: TaskExecutor = {
  async execute(
    ...
    try {
      const operation = (task.metadata?.['operation'] as string) || 'status';
...
export const testExecutor: TaskExecutor = {
  async execute(
    ...
    try {
      const testFiles = (task.metadata?.['testFiles'] as string[]) || [];
...
export const aiCompletionExecutor: TaskExecutor = {
  async execute(
    ...
    try {
      const context = (task.metadata?.['context'] as string) || '';
