 ...
 * Existing `window.electron.store.get(...)` calls resolve to the app's SQLite
 * state table on D:\, avoiding Tauri's user-profile app-data directory.
 *
...
export async function installTauriShim(): Promise<void> {
  ...
  try {
    const [dialogMod, shellMod, pathMod] = await Promise.all([
    ...
    type StateQueryResult = { success: boolean; data?: Array<Record<string, unknown>> };
    async function stateQuery(sql: string, queryParams: string[] = []) {
      return await invoke<StateQueryResult>('db_execute_query', { sql, queryParams });
    }
    ...
    const shim = {
      ...
      store: {
        async get(key: string) {
          const result = await stateQuery('SELECT state_value FROM app_state WHERE state_key = ?', [
            key,
          ]);
          return result.data?.[0]?.['state_value'] ?? undefined;
        },
        ...
        async delete(key: string) {
          await stateQuery('DELETE FROM app_state WHERE state_key = ?', [key]);
          return { success: true };
        },
        async keys() {
          const result = await stateQuery('SELECT state_key FROM app_state ORDER BY state_key');
          return (result.data ?? []).map(row => String(row['state_key']));
        },
      },
      ...
      app: {
        async getPath(name: string) {
          try {
            let p: string;
            if (name === 'userData' || name === 'appData') p = 'D:\\data\\vibe-code-studio';
            else if (name === 'home') p = await pathMod.homeDir();
            else p = 'D:\\data\\vibe-code-studio';
      ...
      db: {
        ...
        async recordLearningOutcome(outcome: unknown) {
          const { invoke } = await import('@tauri-apps/api/core');
          return await invoke('db_record_learning_outcome', { outcome });
        },
        async recordAgentTransition(input: unknown) {
          const { invoke } = await import('@tauri-apps/api/core');
          return await invoke('db_record_agent_transition', { input });
        },
        async recordAgentTerminal(input: unknown) {
          const { invoke } = await import('@tauri-apps/api/core');
          return await invoke('db_record_agent_terminal', { input });
        },
        async getResumableAgentTasks(limit = 10) {
          const { invoke } = await import('@tauri-apps/api/core');
          return await invoke('db_get_resumable_agent_tasks', { limit });
        },
        async getAgentChatOutcomes(taskId: string, limit = 10) {
          const { invoke } = await import('@tauri-apps/api/core');
          return await invoke('db_get_agent_chat_outcomes', { taskId, limit });
        },
        async flushAgentLearningOutbox() {
          const { invoke } = await import('@tauri-apps/api/core');
          return await invoke('db_flush_agent_learning_outbox');
        },
        async close() {
          return { success: true };
        },
          ...
          try {
            ...
            return {
              ...
              stats: {
                ...
                created: info.birthtime ?? new Date(),
      ...
      apex: {
        ...
        async semanticSearch(_query: string, _topK?: number) {
          return [];
        },
        async getStatus() {
          return { initialized: false, indexedFiles: 0 };
        },
        async queryVector(_query: string, _maxResults?: number) {
          return [];
        },
        ...
        async invoke(channel: string, ...args: unknown[]) {
          ...
          switch (channel) {
            case 'db:savePattern': {
              ...
              return await invoke('db_save_pattern', {
    ...
    installed = true;
    void invoke('db_flush_agent_learning_outbox').catch(err => {
      logger.warn('[TauriShim] Pending agent learning outcomes remain queued:', err);
    });
  } catch (err) {
