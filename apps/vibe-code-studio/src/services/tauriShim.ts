...
type StoreInstance = Awaited<ReturnType<typeof import('@tauri-apps/plugin-store').load>>;
...
function buildStoreApi(getStore: () => Promise<StoreInstance>) {
  return {
...
function buildDialogApi(dialogMod: DialogMod) {
  return {
...
function buildShellApi(shellMod: ShellMod) {
  return {
...
function buildAppApi(platformInfo: PlatformInfo, pathMod: PathMod) {
  return {
...
function buildPlatformInfo(homedir: string): PlatformInfo {
...
function assembleShim(deps: {
  getStore: () => Promise<StoreInstance>;
  dialogMod: DialogMod;
  shellMod: ShellMod;
  pathMod: PathMod;
  platformInfo: PlatformInfo;
}) {
  const { getStore, dialogMod, shellMod, pathMod, platformInfo } = deps;
  const shim = {
    ...
    // --- Store ---
    store: buildStoreApi(getStore),

    // --- Dialog ---
    dialog: buildDialogApi(dialogMod),

    // --- Shell ---
    shell: buildShellApi(shellMod),

    // --- App ---
    app: buildAppApi(platformInfo, pathMod),
    ...
    ipcRenderer: buildIpcRendererApi(),
...
export async function installTauriShim(): Promise<void> {
  ...
  try {
    ...
    const platformInfo = buildPlatformInfo(homeDirectory);

    const shim = assembleShim({ getStore, dialogMod, shellMod, pathMod, platformInfo });
