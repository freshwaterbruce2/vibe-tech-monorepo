...
interface FileSystemServiceLike {
  readFile: (filePath: string) => Promise<string | null>;
  writeFile: (filePath: string, content: string) => Promise<void>;
}

export class SearchService {
  private fileSystemService: FileSystemServiceLike;

  constructor(fileSystemService: FileSystemServiceLike) {
  ...
  private formatCsvResults(results: Record<string, SearchResult[]>): string {
  ...
  exportResults(results: Record<string, SearchResult[]>, format: 'json' | 'csv' | 'txt' = 'json'): string {
    switch (format) {
      ...
      case 'csv':
        return this.formatCsvResults(results);
