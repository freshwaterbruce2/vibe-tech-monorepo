...
export interface AIProvider {
  name: 'openrouter' | 'deepseek' | 'openai' | 'anthropic' | 'google';
  apiKey: string;
  baseUrl: string;
  models: string[];
}
...
export class DeepSeekStreamingClient extends BaseStreamingClient {
  async streamCompletion(messages: AIMessage[], options: StreamingOptions): Promise<void> {
    ...
    try {
      ...
      const response = await fetch(`${this.provider.baseUrl}/api/openrouter/chat`, fetchOptions);
...
export class AIProviderFactory {
  static createProvider(config: {
    ...
    const provider: AIProvider = {
      name: config.name as 'openrouter' | 'deepseek' | 'openai' | 'anthropic' | 'google',
  ...
  private static getDefaultBaseUrl(provider: string): string {
    const urls: Record<string, string> = {
      openrouter: import.meta.env['VITE_OPENROUTER_PROXY_URL'] ?? 'http://localhost:3001',
...
const openRouterClient = AIProviderFactory.createProvider({
  name: 'openrouter',
  apiKey: import.meta.env['VITE_OPENROUTER_API_KEY'] ?? '',
});

streamingAI.registerProvider('openrouter', openRouterClient);
