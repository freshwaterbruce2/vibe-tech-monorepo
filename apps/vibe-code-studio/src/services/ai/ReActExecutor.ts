...
function extractJsonCodeFenceBlocks(text: string): string[] {
  const blocks: string[] = [];
...
function tryParseJsonObjectFromText(text: string, expectedKeys: string[]): JsonObject | undefined {
  const rawCandidates = [
    ...extractJsonCodeFenceBlocks(text),
    ...extractBalancedBraceCandidates(text),
  ];

  const uniqueCandidates = Array.from(new Set(rawCandidates)).filter(Boolean);

  let best: { score: number; length: number; parsed: JsonObject } | undefined;
...
**Response Format (JSON only):**
{
  ...
  "expectedOutcome": "What I expect to happen when I execute this"
    ...
    try {
      ...
      const thoughtData = tryParseJsonObjectFromText(response.content, [
        'reasoning',
        'approach',
        'alternatives',
        'confidence',
...
**Response Format (JSON only):**
{
  ...
  "unexpectedEvents": ["Unexpected event 1", "Unexpected event 2"]
    ...
    try {
      ...
      const obsData = tryParseJsonObjectFromText(response.content, [
        'actualOutcome',
        'differences',
        'learnings',
        'unexpectedEvents',
...
**Response Format (JSON only):**
{
  ...
  "knowledgeGained": "Key insight learned from this attempt"
    ...
    try {
      ...
      const refData = tryParseJsonObjectFromText(response.content, [
        'whatWorked',
        'whatFailed',
        'rootCause',
        'shouldRetry',
