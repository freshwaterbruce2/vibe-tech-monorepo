...
import type { LiveEditorStream } from '../../LiveEditorStream';
import type { WorkspaceService } from '../../WorkspaceService';
import type { MetacognitiveLayer } from '../MetacognitiveLayer';
import type { ReActExecutor } from '../ReActExecutor';
...
export class NonRetryableError extends Error {
  constructor(
    message: string,
    public readonly reason: string = 'non-retryable'
  ) {
    super(message);
...
export interface ActionContext {
  ...
  requestMutationApproval?: (step: AgentStep, request: ApprovalRequest) => Promise<boolean>;
}
...
export type { ActionType, AgentStep, AgentTask, ApprovalRequest, StepAction, StepResult };
