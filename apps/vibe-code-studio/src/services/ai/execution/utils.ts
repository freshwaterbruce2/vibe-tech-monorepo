...
export function resolveFilePath(
  ...
  if (!workspaceRoot) {
    throw new Error('Cannot access files without an active workspace root');
  }
...
/** Reject traversal, foreign drives, UNC paths, and prefix-collision escapes. */
export function assertWorkspacePath(candidate: string, workspaceRoot: string): void {
  const normalize = (value: string) => {
    const windowsPath = value.replace(/\//g, '\\');
    const prefix = windowsPath.match(/^[a-z]:/i)?.[0] ?? '';
    const segments = windowsPath.slice(prefix.length).split('\\');
    const canonical: string[] = [];
    for (const segment of segments) {
      if (!segment || segment === '.') continue;
      if (segment === '..') canonical.pop();
      else canonical.push(segment);
    }
    return `${prefix}${prefix ? '\\' : ''}${canonical.join('\\')}`
      .replace(/\\+$/g, '')
      .toLowerCase();
  };
  const root = normalize(workspaceRoot);
  const target = normalize(candidate);
  if (!root || target.startsWith('\\\\') || (target !== root && !target.startsWith(`${root}\\`))) {
    throw new Error(`Path is outside the active workspace: ${candidate}`);
  }
}
...
export function extractCodeFromResponse(response: string, _targetLanguage?: string): string {
  ...
  const unmatchedOpeningFence = trimmed.match(/^```[^\r\n]*\r?\n/);
  if (unmatchedOpeningFence) {
    return trimmed
      .slice(unmatchedOpeningFence[0].length)
      .replace(/\r?\n```\s*$/, '')
      .trim();
  }
  return trimmed;
...
export default function ${fileName.replace(/[.-]/g, '_').replace(/\..+$/, '')}() {
  // Implementation auto-generated by Agent Mode
