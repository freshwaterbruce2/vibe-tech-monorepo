...
export const IpcBridge = {
  ...
  async sendTaskActivity(payload: TaskActivityPayload, target?: string) {
    const msg = createTaskActivityMessage(SOURCE, payload);
    return send(withTarget(msg, target));
  },

  async sendFileOpen(payload: OpenFilePayload, target?: string) {
    const msg = createOpenFileMessage(SOURCE, payload);
    return send(withTarget(msg, target));
  },

  async sendFileChanged(payload: FileChangedPayload, target?: string) {
    // No helper exists in shared-ipc for file_changed; craft manually
    const msg = {
      type: IPCMessageType.FILE_CHANGED,
      source: SOURCE,
      timestamp: Date.now(),
      messageId: `${SOURCE}-${crypto.randomUUID()}`,
      payload,
  ...
  async sendCommandRequest(text: string, target?: string, metadata?: Record<string, unknown>) {
    const msg = {
