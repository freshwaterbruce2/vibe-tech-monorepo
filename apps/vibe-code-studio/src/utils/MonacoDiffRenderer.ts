...
import type { DiffChange } from '../services/LiveEditorStream';
...
export class MonacoDiffRenderer {
  ...
  renderDiff(changes: DiffChange[], options: DiffRenderOptions = {}): void {
    ...
    if (firstChange) {
      this.editor.revealLineInCenter(firstChange.startLine);
    }
  }
  ...
  private calculateInlineDiff(
    ...
    while (i < oldText.length || j < newText.length) {
      if (i >= oldText.length) {
        ...
        const oldMatch = newText.indexOf(oldChar, j);
        const newMatch = oldText.indexOf(newChar, i);
