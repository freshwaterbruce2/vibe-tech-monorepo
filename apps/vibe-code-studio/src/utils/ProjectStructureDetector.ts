...
export class ProjectStructureDetector {
  ...
  async detectStructure(workspaceRoot: string): Promise<ProjectStructure> {
    ...
    // Skip demo:// virtual paths - they're handled in-memory
    const isDemoPath = workspaceRoot?.startsWith('demo://');

    // Check for real Windows filesystem paths (C:\, D:\, etc.)
    ...
    // For demo paths in web mode, return demo structure
    if (isWebMode && isDemoPath) {
      return {
        type: 'nodejs',
