...
export const TerminalPanel = ({ isOpen, onClose }: TerminalPanelProps) => {
  const initialTab = useMemo(() => createTerminalTab(1), []);
  const [terminals, setTerminals] = useState<TerminalTab[]>(() => [initialTab]);
  const [activeTerminalId, setActiveTerminalId] = useState<string | null>(() => initialTab.id);
  const [isMaximized, setIsMaximized] = useState(false);
  const terminalRefs = useRef<Map<string, HTMLDivElement>>(new Map());
  const initializedTerminalIdsRef = useRef<Set<string>>(new Set());

  const createNewTerminal = () => {
    const newTab = createTerminalTab(terminals.length + 1);

    setTerminals(prev => [...prev, newTab]);
    setActiveTerminalId(newTab.id);
  };

  const ensureTerminalStarted = (tab: TerminalTab, element: HTMLDivElement) => {
    if (initializedTerminalIdsRef.current.has(tab.id)) {
      return;
    }
    ...
    initializedTerminalIdsRef.current.add(tab.id);
  };
  ...
  const closeTerminal = (id: string) => {
    ...
    if (tab) {
      ...
      initializedTerminalIdsRef.current.delete(id);
    }
