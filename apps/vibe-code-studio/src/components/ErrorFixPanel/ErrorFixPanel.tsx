  ...
  return (
    ...
    >
      ...
      <Header>
        ...
        <CloseButton onClick={onDismiss} aria-label="dismiss" title="Close (ESC)">
          <X size={20} />
        </CloseButton>
      </Header>
      ...
      <ErrorSection>
        <ErrorHeader>
          <ErrorBadge $severity={error.severity}>{error.type.toUpperCase()}</ErrorBadge>
      ...
      {fix && !isLoading && (
        <Actions>
          <ApplyButton onClick={handleApplyClick} disabled={!selectedSuggestion} aria-label="apply">
