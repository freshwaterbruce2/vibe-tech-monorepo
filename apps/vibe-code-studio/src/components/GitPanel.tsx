...
export const GitPanel = ({ workingDirectory }: GitPanelProps) => {
  const {
    isGitRepo,
    status,
    branches,
  ...
  return (
    <PanelContainer>
      <PanelHeader>
        ...
        <Actions>
          <ActionButton onClick={async () => pull()} title="Pull">
            <Download />
          </ActionButton>
          <ActionButton onClick={async () => push()} title="Push">
            <Upload />
          </ActionButton>
            ...
            <FileList>
              {status.modified.map((file) => (
                <FileItem key={file} $status="modified">
                  ...
                  <FileActions>
                    <ActionButton onClick={async () => handleStageFile(file)} title="Stage">
                      <Plus size={14} />
                    </ActionButton>
                    <ActionButton onClick={async () => handleDiscardChanges(file)} title="Discard">
                      <X size={14} />
                    </ActionButton>
                  </FileActions>
                </FileItem>
            ...
            <FileList>
              {stagedFiles.map((file) => (
                <FileItem key={file} $status="added">
                  <FileName>{file}</FileName>
                  <FileActions>
                    <ActionButton onClick={async () => handleUnstageFile(file)} title="Unstage">
                      <Minus size={14} />
                    </ActionButton>
                  </FileActions>
                </FileItem>
