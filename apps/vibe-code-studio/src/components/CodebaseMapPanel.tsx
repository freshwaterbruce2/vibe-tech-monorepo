...
import type { DependencyGraph, GraphLink as _GraphLink, GraphNode } from '../services/DependencyGraphService';
...
export const CodebaseMapPanel = ({
  dependencies,
  onNodeClick,
  onClose
}: CodebaseMapPanelProps) => {
  ...
  const graphService = useMemo(() => {
    ...
    service.buildGraph(dependencies);
    return service;
  }, [dependencies]);

  const filteredGraph = useMemo<DependencyGraph>(() => {
    const baseGraph = graphService.getGraph();
    const searchLower = searchTerm.trim().toLowerCase();

    const filteredNodes = baseGraph.nodes.filter((node) => {
      if (fileTypeFilter !== 'all' && node.type !== fileTypeFilter) {
        return false;
      }

      if (minDegreeFilter > 0 && node.degree < minDegreeFilter) {
        return false;
      }
    ...
    const filteredLinks = baseGraph.links.filter(
  ...
  useEffect(() => {
    ...
    const node = g.append('g')
      ...
      .join('circle')
      .attr('r', (d: GraphNode) => sizeScale(d.importance))
    ...
    node.on('click', (event: MouseEvent, d: GraphNode) => {
      ...
      node.attr('opacity', (n: GraphNode) => connectedNodes.has(n.id) ? 1 : 0.3);
      link.attr('opacity', (l: any) =>
        (l.source as any).id === d.id || (l.target as any).id === d.id ? 1 : 0.1
      );
      label.attr('opacity', (n: GraphNode) => connectedNodes.has(n.id) ? 1 : 0.3);
    });
    ...
    // Drag functions - D3 simulation node manipulation (not React state)
