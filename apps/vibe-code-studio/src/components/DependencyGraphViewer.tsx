...
import type { DependencyGraph } from '@vibetech/types/multifile';
...
export const DependencyGraphViewer = ({
  ...
  const [highlightLinks, setHighlightLinks] = useState<string[]>([]);
  ...
  const handleNodeClick = (node: any) => {
    ...
    setHighlightLinks(Array.from(connectedLinks));
  };
    ...
    >
      ...
      <GraphContainer>
        <ForceGraph2D
          ...
          linkColor={(link: any) =>
            highlightLinks.includes(`${link.source}-${link.target}`)
              ? 'rgba(139, 92, 246, 0.8)'
              : 'rgba(139, 92, 246, 0.3)'
          }
          linkWidth={(link: any) =>
            highlightLinks.includes(`${link.source}-${link.target}`) ? 2 : 1
          }
          ...
          nodeCanvasObject={(node: any, ctx, globalScale) => {
            ...
            // Highlight selected
            if (selectedNode === node.id || highlightNodes.includes(node.id)) {
              ctx.strokeStyle = '#fff';
      ...
      {selectedNode && (
        <InfoPanel>
          ...
          <InfoText>
            Connected: {highlightNodes.length - 1} nodes
          </InfoText>
        </InfoPanel>
      )}
    </Container>
