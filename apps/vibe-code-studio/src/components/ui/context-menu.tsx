...
export const ContextMenu = ({ items, x, y, onClose }: ContextMenuProps) => {
  ...
  const position = useMemo(() => {
    ...
    const estimatedMenuHeight = Math.max(200, items.length * 34);
    const maxX = Math.max(10, window.innerWidth - estimatedMenuWidth - 10);
    const maxY = Math.max(10, window.innerHeight - estimatedMenuHeight - 10);
