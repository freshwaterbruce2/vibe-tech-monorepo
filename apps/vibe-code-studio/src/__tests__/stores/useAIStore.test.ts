...
describe('useAIStore', () => {
  beforeEach(() => {
    ...
    act(() => {
      ...
      // Also clear conversation history for clean tests
      const state = useAIStore.getState();
      useAIStore.setState({ ...state, conversationHistory: [] });
    });
  });
  ...
  describe('model selection', () => {
    it('should set current model', () => {
      ...
      act(() => {
        result.current.actions.setModel('anthropic/claude-sonnet-4.6');
      });
    ...
    it('should fall back to default model for unknown IDs', () => {
      const { result } = renderHook(() => useAIStore());
  ...
  describe('conversation history', () => {
    ...
    it('should load conversation', () => {
      ...
      // Get the conversation ID after state has updated
      const conversationId = result.current.conversationHistory[0]?.id;
