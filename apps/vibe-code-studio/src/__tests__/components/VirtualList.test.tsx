...
import { useVirtualScroll } from '@/components/useVirtualScroll';
...
describe('VirtualList', () => {
  ...
  it('applies custom className', () => {
    const { container } = render(<VirtualList {...defaultProps} className="custom-virtual-list" />);
...
describe('useVirtualScroll', () => {
  it('calculates visible range correctly', () => {
    ...
    const { result } = renderHook(() => useVirtualScroll(items, 400, 50, 3));
  ...
  it('calculates total height', () => {
    ...
    const { result } = renderHook(() => useVirtualScroll(items, 400, 40));
  ...
  it('handles dynamic item heights in hook', () => {
    ...
    const { result } = renderHook(() => useVirtualScroll(items, 400, dynamicHeight));
  ...
  it('updates scroll position', () => {
    const items = generateItems(100);
    const { result } = renderHook(() => useVirtualScroll(items, 400, 50));
