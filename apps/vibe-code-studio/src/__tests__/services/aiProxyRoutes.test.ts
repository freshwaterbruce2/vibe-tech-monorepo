...
const AI_KEYS = [
  'MOONSHOT_API_KEY', 'KIMI_API_KEY', 'VITE_MOONSHOT_API_KEY', 'VITE_KIMI_API_KEY',
  'GOOGLE_API_KEY', 'GEMINI_API_KEY', 'VITE_GOOGLE_API_KEY',
...
function makeRes() {
  const res: {
    status?: number; headers?: unknown; body?: string;
    headersSent: boolean; writableEnded: boolean;
    writeHead: (s: number, h?: unknown) => void;
    end: (b?: string) => void; write: () => void;
    on: () => unknown; removeListener: () => unknown; destroy: () => void;
  } = {
    headersSent: false,
    writableEnded: false,
    writeHead(status: number, headers?: unknown) { res.status = status; res.headers = headers; res.headersSent = true; },
    end(body?: string) { res.body = body; res.writableEnded = true; },
    write() { /* streaming chunk */ },
    on() { return res; },
    removeListener() { return res; },
    destroy() { res.writableEnded = true; },
  };
  return res;
}
...
describe('ai-proxy route handler', () => {
  ...
  describe('auth gate', () => {
    it('returns 401 for an AI call without a session', async () => {
      process.env['KIMI_API_KEY'] = 'k-123';
      const res = makeRes();
      await registerAiProxyRoutes(req('POST', '/api/ai/moonshot/v1/chat/completions'), res, ctxBase);
    ...
    it('returns 401 when the session user no longer exists (revocation check)', async () => {
      ...
      await registerAiProxyRoutes(
        req('POST', '/api/ai/moonshot/v1/chat/completions', 'sid=stale'), res, ctxBase
      );
      expect(res.status).toBe(401);
    });
  });
  ...
  describe('provider configuration gate', () => {
    it('returns 503 when no key is configured for the provider', async () => {
      const res = makeRes();
      await registerAiProxyRoutes(
        req('POST', '/api/ai/moonshot/v1/chat/completions', 'sid=good'), res, ctxBase
      );
