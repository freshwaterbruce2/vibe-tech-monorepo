...
// Minimal mock to satisfy test usage when better-sqlite3 is unavailable
class MockDB {
  exec(_sql: string) { }
  prepare(_sql: string) {
    return {
...
try {
  // Dynamic import for ESLint compliance with optional dependencies
...
describe.skip('DatabaseService - Real Integration Tests', () => {
  beforeEach(() => {
    initializeDatabase();
  ...
  describe('Chat History Operations', () => {
    ...
    it('should retrieve chat history by workspace', () => {
      ...
      stmt.run('/test/workspace1', 'Question 1', 'Answer 1', 'deepseek/deepseek-v3.2');
      stmt.run('/test/workspace1', 'Question 2', 'Answer 2', 'deepseek/deepseek-v3.2');
      stmt.run('/test/workspace2', 'Question 3', 'Answer 3', 'openai/gpt-5.2-codex');
  ...
  describe('Analytics & Telemetry', () => {
    ...
    it('should retrieve events by type', () => {
      ...
      db.prepare(`
        INSERT INTO analytics_events (event_type, event_data)
        VALUES (?, ?)
      `).run('code_generated', '{"model": "deepseek/deepseek-v3.2"}');

      db.prepare(`
        INSERT INTO analytics_events (event_type, event_data)
        VALUES (?, ?)
      `).run('code_generated', '{"model": "openai/gpt-5.2-codex"}');
