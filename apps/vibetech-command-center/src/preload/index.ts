...
async function ensureWs(): Promise<void> {
  ...
  if (ws?.readyState === WebSocket.CONNECTING) {
    ...
    await new Promise<void>((resolve) => {
      ...
      connectingSocket.addEventListener(
        'open',
        () => {
          clearTimeout(t);
          resolve();
  ...
  if (wsPort === null) {
    const info = (await ipcRenderer.invoke(IPC_CHANNELS.META_INFO)) as {
      ok: boolean;
      data?: { wsPort: number };
    };
...
const api: CommandCenterAPI = {
  ...
  dbExplorer: {
    ...
    query: async (dbPath: string, sql: string) =>
      ipcRenderer.invoke(IPC_CHANNELS.DB_EXPLORER_QUERY, dbPath, sql),
  },
  agent: {
  ...
  stream: {
    subscribe(topic, handler) {
      ...
      set.add(handler);
      void ensureWs().catch(() => {
        /* renderer receives nothing until reconnect */
      });
      return () => {
