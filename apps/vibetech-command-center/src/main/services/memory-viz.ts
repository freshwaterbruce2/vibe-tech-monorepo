...
export class MemoryVizService {
  ...
  getSnapshot(): unknown {
    ...
    const proceduralCount = (
      db.prepare('SELECT COUNT(*) as count FROM procedural_memory').get() as { count: number }
    ).count;

    let embeddingDim = 0;
    try {
      const row = db
        .prepare('SELECT embedding FROM semantic_memory WHERE embedding IS NOT NULL LIMIT 1')
    ...
    try {
      const logRow = db
        .prepare('SELECT MAX(created_at) as lastRun FROM consolidation_log')
        .get() as { lastRun: number } | undefined;
      if (logRow?.lastRun) lastRunAt = logRow.lastRun;
      itemsSummarized = (
        db
          .prepare("SELECT COUNT(*) as count FROM consolidation_log WHERE action = 'merge'")
          .get() as { count: number }
      ).count;
      itemsPruned = (
        db
          .prepare("SELECT COUNT(*) as count FROM consolidation_log WHERE action = 'prune'")
    ...
    `,
      ...
      .all(RECENT_MEMORY_LIMIT) as Array<{
      id: number;
      sourceId: string;
      timestamp: number;
      query: string;
    ...
    return {
      stats: [
        ...
        {
          store: 'semantic',
          recordCount: semanticCount,
          avgEmbeddingDim: embeddingDim || undefined,
        },
  ...
  search(query: string, topK = 10): unknown[] {
    ...
    try {
      ...
      `,
        ...
        .all(pattern, pattern, pattern, topK) as Array<{
        query: string;
        response: string;
        score: number;
      }>;
    ...
    try {
      ...
      for (const r of rows)
        results.push({ source: 'procedural', score: r.score, text: `${r.pattern}: ${r.context}` });
    } catch {
      /* ignore */
    }
  ...
  computeDecay(): unknown[] {
    ...
    `,
      ...
      .all(DECAY_PREVIEW_LIMIT) as Array<{
      id: number;
      text: string;
      category: string | null;
      importance: number;
