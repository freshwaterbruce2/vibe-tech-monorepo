  ...
  const filteredConfigs = useMemo(() => {
    return configs.filter((c) => {
      const matchesSearch = c.projectName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                            c.projectRoot.toLowerCase().includes(searchTerm.toLowerCase());
          ...
          {activeProject ? (
            <div className="rounded-xl border border-bg-line bg-bg-elev p-5 space-y-5">
              ...
              <form onSubmit={handleUpdateVar} className="border-t border-bg-line pt-4 space-y-3">
                <div className="text-xs font-semibold text-slate-300">Set Environment Variable</div>
                <div className="grid gap-2">
                  <input
                    value={newKey}
                    onChange={(e) => setNewKey(e.target.value)}
                    placeholder="KEY (e.g. STRIPE_SECRET_KEY)"
                    className="w-full bg-bg-panel border border-bg-line rounded px-3 py-1.5 text-xs text-slate-200 focus:outline-none focus:border-pulse-cyan"
                    required
                  />
                  <input
                    value={newValue}
