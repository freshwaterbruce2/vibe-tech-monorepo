  ...
  return (
    <div className="flex flex-col gap-4 h-full min-h-0">
      <div className="shrink-0 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3 text-xs">
          <span className="flex items-center gap-1 text-status-ok">
            <Activity size={12} /> {healthyCount} healthy
          </span>
          <span className="flex items-center gap-1 text-status-error">
            <Activity size={12} /> {unhealthyCount} unhealthy
          </span>
        </div>
      </div>
      <div className="flex-[2] min-h-0 flex flex-col gap-4 overflow-hidden">
        ...
        <NxTaskLauncher
          onLaunch={(spec) => taskRun.mutate(spec)}
          isPending={taskRun.isPending}
          lastHandle={taskRun.data ?? null}
        />
      </div>
...
function McpHealthStrip({
  ...
  if (isLoading && servers.length === 0)
    return <div className="text-slate-500 text-sm">loading mcp status...</div>;
  return (
    <div className="shrink-0 space-y-2">
      ...
      <div className="flex flex-wrap gap-2">
        {servers.map((s) => {
          ...
          return (
            <div key={s.name} className="relative">
              <button
                onClick={() => handleSelect(s.name, isSelected)}
                className={clsx(
              ...
              >
                <span className="flex items-center gap-1.5">
                  <Server size={12} />
                  {s.name}
                  <span className="opacity-75">({s.transport})</span>
                </span>
              </button>
              {isSelected && (
                <div className="absolute z-10 mt-1 w-56 bg-bg-elev border border-bg-line rounded-lg p-3 shadow-glow-cyan">
                  <div className="text-xs text-slate-300 space-y-1">
                    ...
                    {s.runtimeStatus && (
                      <div>
                        status: <span className="font-mono">{s.runtimeStatus}</span>
                      </div>
                    )}
                    <div>
                      probed: <RelativeTime ts={s.lastProbeAt} />
                    </div>
        ...
        {servers.length === 0 && (
          <span className="text-slate-500 text-xs italic">no mcp servers configured</span>
        )}
      </div>
    </div>
  );
...
function NxTaskLauncher({
  onLaunch,
  isPending,
  lastHandle,
}: {
  onLaunch: (spec: AgentTaskLauncher) => void;
  isPending: boolean;
  lastHandle: ProcessHandle | null;
}) {
  ...
  const projects = useMemo(
    () => Object.values(nxQuery.data?.projects ?? {}).sort((a, b) => a.name.localeCompare(b.name)),
    [nxQuery.data],
  );
    ...
    <div className="bg-bg-elev border border-bg-line rounded-lg p-4 shrink-0">
      ...
      <form onSubmit={handleSubmit} className="flex flex-wrap items-end gap-3">
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            className="bg-bg-panel border border-bg-line rounded px-2 py-1.5 text-sm text-slate-200 min-w-[160px]"
          >
        ...
        <div className="flex flex-col gap-1">
          ...
          <select
            ...
            className="bg-bg-panel border border-bg-line rounded px-2 py-1.5 text-sm text-slate-200 min-w-[120px]"
          >
            {TARGETS.map((t) => (
        ...
        <div className="flex flex-col gap-1 flex-1 min-w-[120px]">
          ...
          <input
            ...
            placeholder="--skip-nx-cache"
            className="bg-bg-panel border border-bg-line rounded px-2 py-1.5 text-sm text-slate-200"
          />
        </div>
        <button type="submit" disabled={isPending || !project} className="btn btn-primary">
          <Play size={14} />
      ...
      {lastHandle && (
        <div className="mt-2 text-xs">
          <span className="text-status-ok">Launched</span> process{' '}
          <span className="font-mono text-slate-300">{lastHandle.id}</span> —{' '}
          <span
            className={clsx(
              lastHandle.status === 'running'
                ? 'text-pulse-cyan'
                : lastHandle.status === 'exited' && lastHandle.exitCode === 0
                  ? 'text-status-ok'
                  : 'text-status-error',
            )}
          >
...
function TaskList({
  ...
  const handleRestart = useCallback(
    (task: AgentTaskSpec) => {
      void (async () => {
        await unwrap(
          window.commandCenter.agent.taskRun({
  ...
  return (
    <Panel
      title={`Tasks (${tasks.filter((t) => t.status === 'running').length} running, ${tasks.length} total)`}
      loading={isLoading}
      onRefresh={onRefetch}
    >
      <div className="overflow-auto max-h-[35vh]">
        <table className="w-full text-sm">
          ...
          <tbody>
            ...
            {sorted.length === 0 && (
              <tr>
                <td colSpan={7} className="py-4 text-slate-500 text-sm italic text-center">
...
function TaskRow({
  ...
  onRestart: (task: AgentTaskSpec) => void;
}) {
  const durationMs =
    task.status === 'running' ? now - task.startedAt : (task.exitedAt ?? now) - task.startedAt;
  ...
  return (
    <tr className="border-b border-bg-line hover:bg-bg-elev transition-colors">
      ...
      <td className="py-2 px-3">
        <RelativeTime ts={task.startedAt} />
      </td>
      <td className="py-2 px-3 font-mono text-xs text-slate-400">{durationText}</td>
      <td className="py-2 px-3 font-mono text-xs">
        {task.exitCode !== null ? (
          <span className={task.exitCode === 0 ? 'text-status-ok' : 'text-status-error'}>
            {task.exitCode}
          </span>
        ) : (
        ...
        <div className="flex items-center justify-end gap-2">
          {task.status === 'running' && (
            <button
              ...
              className="text-slate-500 hover:text-status-error"
              title="kill"
          ...
          {(task.status === 'exited' || task.status === 'killed' || task.status === 'error') && (
            <button
              ...
              className="text-slate-500 hover:text-pulse-cyan"
              title="restart"
        ...
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            ...
            <select
              ...
              className="bg-bg-panel border border-bg-line rounded px-2 py-1.5 text-sm text-slate-200 min-w-[140px]"
            >
                ...
                <button
                  ...
                  className={clsx(
                    ...
                    stream === s
                      ...
                      : 'text-slate-400 hover:bg-bg-elev',
                  )}
                  type="button"
                >
                  {s}
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex gap-1">
              {[1, 5, 15].map((m) => (
                <button
                  key={m}
                  onClick={() => setSinceMinutes(sinceMinutes === m ? undefined : m)}
                  className={clsx(
                    'px-2 py-1.5 text-xs rounded border transition-colors',
                    sinceMinutes === m
                      ? 'border-pulse-cyan-700 bg-pulse-cyan-900 text-pulse-cyan'
                      : 'border-bg-line text-slate-400 hover:bg-bg-elev',
                  )}
                  type="button"
                >
                  {m}m
        ...
        <div className="bg-bg-base border border-bg-line rounded p-3 h-48 overflow-auto font-mono text-xs space-y-0.5">
          {results.length === 0 ? (
            <div className="text-slate-600 italic">
              {isSearching ? 'searching...' : 'no results — adjust filters and search'}
            </div>
          ) : (
            results.map((chunk, i) => (
              <div
                ...
                className={clsx(
                  'flex gap-2',
                  chunk.stream === 'stderr' ? 'text-status-error' : 'text-slate-300',
                )}
              >
