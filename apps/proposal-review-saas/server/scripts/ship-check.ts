...
const testAuthDbPath = 'D:\\databases\\proposal-review-test-auth.db';
...
async function main(): Promise<void> {
  ...
  await checkEnvPresence(['RAILWAY_TOKEN'], 'railway-env', 'Railway deploy credentials are present');
...
async function checkFrontendConfig(): Promise<void> {
...
async function runLocalApiSmoke(): Promise<void> {
  ...
  try {
    ...
    const afterLogout = await fetchJson<{ user: unknown }>(`http://${appHost}:${appPort}/api/auth/me`, {
      headers: {
