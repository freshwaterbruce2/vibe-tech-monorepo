...
Write-Host "Setting up ADB reverse proxy for ports $backendPort, $gatewayPort and $metroPort..." -ForegroundColor Cyan
...
$result2 = adb reverse tcp:$gatewayPort tcp:$gatewayPort
$result3 = adb reverse tcp:$metroPort tcp:$metroPort
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "and the gateway at http://localhost:$gatewayPort" -ForegroundColor Green
    ...
    Write-Host "Error details: $result1 $result2 $result3" -ForegroundColor Red
}
