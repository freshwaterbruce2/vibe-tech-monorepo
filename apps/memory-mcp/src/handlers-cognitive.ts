...
export async function handleCognitiveMemory(
  ...
  switch (name) {
    ...
    case 'memory_cognitive_retrieve': {
      ...
      const _t0 = Date.now();
      const cognitiveResults = await memoryManager.cognitive.retrieve(query, {
        limit,
        kind,
        minScore,
      });
      ...
      return {
        content: [{
          ...
          text: JSON.stringify({
            ...
            results: cognitiveResults.map((result) => ({
              id: result.record.id,
            ...
            semanticResults: semanticResults.map((result: SearchResult<SemanticMemory>) => ({
              id: result.item.id,
