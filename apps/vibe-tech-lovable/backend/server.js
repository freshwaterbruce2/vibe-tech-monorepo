...
app.post('/api/customers', (req, res) => {
  ...
  db.run(
    ...
    (err) => {
      ...
      db.get('SELECT * FROM customers WHERE id = ?', [id], (getErr, row) => {
        if (getErr) {
...
app.post('/api/invoices', (req, res) => {
  ...
  db.run(
    ...
    (err) => {
      ...
      db.get('SELECT * FROM invoices WHERE id = ?', [id], (getErr, row) => {
        if (getErr) {
          return res.status(500).json({ error: getErr.message });
        }
        return res.json(row);
...
app.post('/api/leads', (req, res) => {
  ...
  db.run(
    ...
    (err) => {
      ...
      db.get('SELECT * FROM leads WHERE id = ?', [id], (getErr, row) => {
        if (getErr) {
          return res.status(500).json({ error: getErr.message });
        }
        return res.json(row);
...
app.post('/api/blog', (req, res) => {
  ...
  `, [
    ...
    db.get('SELECT * FROM blog_posts WHERE id = ?', [id], (getErr, row) => {
      if (getErr) {
        return res.status(500).json({ error: getErr.message });
      }
      const post = {
...
app.patch('/api/blog/:id/publish', (req, res) => {
  ...
  db.run(
    ...
    function(err) {
      ...
      return res.json({ published: Boolean(published), published_at });
    }
  );
});
