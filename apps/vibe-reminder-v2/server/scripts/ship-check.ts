...
const appPort = 6330 + 2;
const appHost = '127.0.0.1';
const appBaseUrl = 'http://127.0.0.1:5330';
const authSecret = 'appointment-reminder-v2-local-auth-secret-12345';
const operatorEmail = 'owner@example.com';
const operatorPassword = 'change-this-password';
const operatorName = 'AppointmentReminderV2 Owner';
...
const testAuthDbPath = 'D:\\databases\\vibe-reminder-v2-test-auth.db';
...
async function runLocalApiSmoke(): Promise<void> {
  ...
  const env = {
    ...
    APPOINTMENTS_DATABASE_PATH: 'D:\\databases\\appointment-reminder-v2-test.db',
  ...
  try {
    ...
    const receipt = await fetchJson<{ subject?: string; html?: string; text?: string }>(
      `http://${appHost}:${appPort}/api/emails/demo-receipt`,
    );
    const receiptOk =
      typeof receipt.subject === 'string' &&
      receipt.subject.length > 0 &&
      typeof receipt.html === 'string' &&
      receipt.html.includes('DEMO-100') &&
      typeof receipt.text === 'string' &&
      receipt.text.includes('DEMO-100');
    results.push({
      id: 'api-email-receipt',
      ok: receiptOk,
      detail: receiptOk
        ? 'Receipt email route renders shared email subject, HTML, and text output'
        : 'Receipt email route did not render the expected shared email payload',
    });

    const aiUsage = await fetchJson<{ usage?: { totalTokens?: number } }>(
      `http://${appHost}:${appPort}/api/ai/demo-usage`,
      {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
    ...
    results.push({
      id: 'api-ai-usage',
      ok: aiUsage.usage?.totalTokens === 200,
      detail:
        aiUsage.usage?.totalTokens === 200
          ? 'Protected AI usage route records shared metering state'
