...
export const openDb = (): Database.Database => {
  ...
  // Safely check if database schema already exists
  const tableCheck = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='appointments'").get() as { name: string } | undefined;
  
  if (!tableCheck) {
    console.log(`Database table 'appointments' not found at ${dbPath}. Initializing empty schema...`);
    migrate(db);
    seedDemoAppointments(db);
  } else {
    console.log(`Successfully connected to active production database at ${dbPath} without applying migrations.`);
  }
