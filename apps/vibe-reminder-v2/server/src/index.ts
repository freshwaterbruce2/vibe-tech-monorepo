...
import { setTenantPlan } from '@vibetech/monetization';
...
const port = Number(process.env.PORT ?? 6330);
...
const stripeWebhookBus = createStripeWebhookBus({
  ...
  handlers: {
    'checkout.session.completed': (event, context) => {
      ...
      setTenantPlan(GENERATED_TENANT_ID, 'pro');
      context.logger?.info?.(
        {
          app: 'appointment-reminder-v2',
          eventId: event.id,
...
app.post('/api/auth/login', async (req, reply) => {
  ...
  const plan = getGeneratedPlan();
  const userWithPlan = { ...user, plan };
  reply.header('set-cookie', buildGeneratedSessionCookie(userWithPlan));
  return {
    ok: true,
    user: userWithPlan,
  };
...
app.get('/api/emails/demo-receipt', async () => {
  ...
  const email = await buildPaymentReceiptEmail({
    ...
    paidAt: '2026-05-15',
    viewUrl,
    companyName: 'AppointmentReminderV2',
...
app.post('/api/pro/rewrite', async (req, reply) => {
  ...
  return {
    ...
    rewrite: {
      recommendedCta:
        'Open the paid AppointmentReminderV2 workflow this week to unlock the premium action.',
    },
  };
});
...
app.get('/api/billing/demo-checkout', async (req, reply) => {
  ...
  try {
    const session = await createTenantCheckoutSession({
      ...
      lineItems: [
        {
          name: 'AppointmentReminderV2 Pro',
          unitAmount: 19,
        },
      ],
    });
...
app.post('/api/ai/demo-usage', async (req, reply) => {
  ...
  return {
    ...
    user: authUser,
    usage,
    summary: getAiUsage('appointment-reminder-v2', 'openrouter', GENERATED_TENANT_ID),
  };
});
