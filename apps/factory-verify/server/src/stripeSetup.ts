  ...
  try {
    ...
    for (const tier of pricingTiers) {
      ...
      let product = existingProducts.data.find(
        (p: any) => p.metadata.appName === 'factory-verify' && p.metadata.plan === tier.key,
      );

      if (!product) {
        logger.info(`Creating Stripe product for ${tier.name}...`);
        product = await stripe.products.create({
          name: `FactoryVerify - ${tier.name}`,
          description: `Pricing plan for ${tier.name} tier`,
          metadata: {
            appName: 'factory-verify',
            plan: tier.key,
          },
        });
      }
    ...
    let customer = existingCustomers.data.find((c: any) => c.metadata.appName === 'factory-verify');

    if (!customer) {
      logger.info(`Creating Stripe Customer for ${email}...`);
      customer = await stripe.customers.create({
        email,
        name: `FactoryVerify Owner`,
        metadata: {
          appName: 'factory-verify',
          role: 'owner',
        },
