...
app.post('/api/pro/rewrite', async (req, reply) => {
  ...
  return {
    ...
    rewrite: {
      recommendedCta:
        'Open the paid FactoryVerify workflow this week to unlock the premium action.',
    },
  };
});
...
app.post('/api/ai/demo-usage', async (req, reply) => {
  ...
  return {
    ...
    user: authUser,
    usage,
    summary: getAiUsage('factory-verify', 'openrouter', GENERATED_TENANT_ID),
  };
});
