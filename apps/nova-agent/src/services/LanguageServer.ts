...
export class LanguageServer {
   ...
   * Open a document
   */
  openDocument(params: { uri: string; languageId: string; version: number; text: string }): void {
   ...
   */
  changeDocument(params: { uri: string; version: number; changes: TextDocumentChange[] }): void {
    ...
    const newDocument = TextDocument.update(document, params.changes, params.version);
   ...
   */
  async getHover(params: { uri: string; position: Position }): Promise<Hover | null> {
    const document = this.documents.get(params.uri);
    if (!document) {
      return null;
    }
   ...
   */
  async getDefinition(params: { uri: string; position: Position }): Promise<Location[] | null> {
    const document = this.documents.get(params.uri);
    if (!document) {
      return null;
    }
  ...
  private findDefinition(document: TextDocument, symbol: string): number {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      const constPattern = new RegExp(`const\\s+${escapedSymbol}\\s*=`);
      const letPattern = new RegExp(`let\\s+${escapedSymbol}\\s*=`);
      const varPattern = new RegExp(`var\\s+${escapedSymbol}\\s*=`);
