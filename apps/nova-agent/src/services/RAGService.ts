...
const RAG_HTTP_BRIDGE = import.meta.env.VITE_MEMORY_MCP_URL ?? 'http://localhost:3200';
let callBridgeId = 0;
...
async function callBridge(tool: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  if (data.result?.content?.[0]?.text) {
    try {
      ...
      console.error(
        'RAG bridge returned invalid JSON:',
        parseError instanceof Error ? parseError.message : parseError,
      );
      throw new Error('RAG bridge returned invalid JSON');
    }
  }
  return data.result;
}
...
export class RAGService {
   ...
   */
  static async search(query: string, topK: number = 5): Promise<RAGSearchResult[]> {
  ...
  static buildContextString(results: RAGSearchResult[]): string {
    ...
    return results
      .map((result, index) => {
        ...
        const similarity = Math.max(0, Math.min(100, (1 - result.distance) * 100)).toFixed(2);
...
`;
   ...
   */
  static async searchAndFormatContext(query: string, topK: number = 3): Promise<string> {
