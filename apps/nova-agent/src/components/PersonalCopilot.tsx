...
import { Code, Lightbulb, Loader2, RefreshCw, Search, TrendingUp } from 'lucide-react';
...
const getCopilotConfig = () => ({
  rootPath: import.meta.env.VITE_COPILOT_ROOT ?? 'V:\\monorepo',
  maxFiles: Number(import.meta.env.VITE_COPILOT_MAX_FILES ?? 2000),
});
...
export default function PersonalCopilot() {
  ...
  const indexCodebase = async () => {
    setIsIndexing(true);
    try {
      const { rootPath, maxFiles } = getCopilotConfig();
  ...
  const markPatternAsUsed = async (patternId: number) => {
    try {
      ...
      showError('Failed to mark pattern as used', error);
    }
  };
        ...
        {stats && (
          <div className="grid grid-cols-3 gap-4 text-sm">
            ...
            <div className="bg-gray-800 p-3 rounded-lg">
              ...
              <div className="text-2xl font-bold">{stats.by_language.length}</div>
            </div>
            <div className="bg-gray-800 p-3 rounded-lg">
      ...
      <div className="flex-1 overflow-y-auto p-4">
            ...
            {suggestions.map((suggestion, idx) => (
              <div key={idx} className="space-y-2">
                <div className="text-sm text-gray-400">
                  {suggestion.reason} (Relevance: {(suggestion.relevance_score * 100).toFixed(0)}%)
                </div>
                <PatternCard
                  ...
                  onCopy={() => copyToClipboard(suggestion.pattern.code_snippet)}
                />
              </div>
            ))}
          </div>
        ...
        {stats?.total_patterns === 0 && (
          <div className="text-center py-12 text-gray-400">
            ...
            <p className="text-sm">Click "Index Codebase" to start learning from your code</p>
          </div>
        )}
      </div>
    </div>
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2 py-1 bg-blue-600 text-xs rounded">{pattern.pattern_type}</span>
            <span className="px-2 py-1 bg-gray-700 text-xs rounded">{pattern.language}</span>
            <span className="font-mono text-sm">{pattern.name}</span>
          </div>
          <div className="text-xs text-gray-400 truncate">{pattern.file_path}</div>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-400">Used {pattern.usage_count}x</span>
          <button
            onClick={onCopy}
