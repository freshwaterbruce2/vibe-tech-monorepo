...
import { AlertCircle, AlertTriangle, Check, CheckCircle, Info, X } from 'lucide-react';
...
const NotificationsPanel = ({ isOpen, onClose }: NotificationsPanelProps) => {
  const { notifications, markAsRead, markAllAsRead, removeNotification, clearAllNotifications } =
    useNotifications();
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <Button
                        variant="ghost"
                        size="icon"
                        className="h-5 w-5 -mt-1 -mr-1 text-gray-400 opacity-50 hover:opacity-100"
                        onClick={(e) => {
                          e.stopPropagation();
