...
describe('AnalysisManager grounded review', () => {
  ...
  beforeEach(() => {
    ...
    process.env.NOVA_WORKSPACE_ROOT = tempRoot;
  });

  afterEach(() => {
    delete process.env.NOVA_REVIEW_ARTIFACT_DIR;
    delete process.env.NOVA_WORKSPACE_ROOT;
  ...
  it('fails on a missing path instead of guessing', async () => {
    ...
    await expect(manager.reviewProject(path.join(tempRoot, 'missing-project'))).rejects.toThrow(
      'does not exist',
    );
  });

  it('rejects paths outside the configured workspace root', async () => {
    const outsidePath = fs.mkdtempSync(path.join(os.tmpdir(), 'nova-review-outside-'));
    try {
      const manager = new AnalysisManager();
      await expect(manager.reviewProject(outsidePath)).rejects.toThrow(
        'Project path must be inside workspace root',
      );
    } finally {
