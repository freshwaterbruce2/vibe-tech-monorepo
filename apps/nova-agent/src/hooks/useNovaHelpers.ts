...
export function useActivityLogger() {
  const logActivity = useCallback(async (activityType: string, details: string) => {
    try {
...
async function createTaskImpl(data: CreateTaskInput): Promise<CreateTaskResult> {
  try {
    return await invoke<CreateTaskResult>('create_task', {
      request: {
        title: data.title,
...
export function useTaskManager() {
  ...
  const createTask = useCallback(async (data: CreateTaskInput) => createTaskImpl(data), []);
