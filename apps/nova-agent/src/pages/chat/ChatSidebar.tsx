...
interface ChatSidebarProps {
  ...
  onRecoveryChange: (candidates: ResumeCandidate[]) => void;
}
...
export function ChatSidebar({
  ...
  const refreshRecovery = async () =>
    onRecoveryChange(AgentService.filterResumeCandidates(await AgentService.getResumeCandidates()));
  ...
  const handleResume = async (candidate: ResumeCandidate) => {
    try {
      await AgentService.resumeTask(candidate);
      await refreshRecovery();
      toast({ title: 'Task queued', description: 'NOVA will continue from the saved checkpoint.' });
    } catch (err) {
      toast({ title: 'Resume blocked', description: String(err), variant: 'destructive' });
    }
  };

  const handleStartOver = async (taskId: string) => {
    try {
      await AgentService.startTaskOver(taskId);
      await refreshRecovery();
      toast({ title: 'Task reset', description: 'The incomplete checkpoint was cleared.' });
    } catch (err) {
      toast({ title: 'Start over blocked', description: String(err), variant: 'destructive' });
    }
  };
  ...
  const handleReconcile = async (
    ...
    const actionId = candidate.uncertain_action_ids[0];
    if (!actionId) return;
    const evidence =
      decision === 'confirm_completed'
        ? (window.prompt('Enter a non-sensitive evidence reference confirming completion') ??
          undefined)
        : undefined;
    if (decision === 'confirm_completed' && !evidence) return;
    try {
      await AgentService.reconcileTaskAction(candidate.task_id, actionId, decision, evidence);
      await refreshRecovery();
      toast({ title: 'Action reconciled', description: 'The recovery decision was saved.' });
    } catch (err) {
      toast({ title: 'Reconciliation blocked', description: String(err), variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="hidden lg:flex w-80 flex-col gap-4">
              ...
              <div key={task.id} className="text-xs bg-white/5 rounded-lg p-3 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => void handleRejectTask(task.id)}
                    className="h-6 text-xs text-red-400 hover:text-red-300 hover:bg-red-900/20 flex-1"
                  >
                    Reject
                  </Button>
                </div>
      ...
      {resumeCandidates.length > 0 && (
        <Card className="p-4 bg-black/40 backdrop-blur-xl border-cyan-500/20 text-white">
          <h3 className="font-bold mb-3 text-cyan-400">Interrupted Tasks</h3>
          <div className="space-y-2">
            {resumeCandidates.map((candidate) => (
              <div key={candidate.task_id} className="text-xs bg-white/5 rounded-lg p-3 space-y-2">
                <p className="text-white/80 truncate">Task {candidate.task_id}</p>
                <p className="text-white/50">
                  {candidate.classification === 'resumable' ? 'Ready to resume' : 'Needs review'}
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => void handleStartOver(candidate.task_id)}
                    className="h-6 text-xs flex-1"
                  >
                    Start over
                  </Button>
                ...
                {candidate.classification === 'needs_review' &&
                  candidate.uncertain_action_ids.length > 0 && (
                    <div className="grid grid-cols-3 gap-1">
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => void handleReconcile(candidate, 'confirm_completed')}
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => void handleReconcile(candidate, 'retry')}
                        className="h-6 text-[10px]"
                      >
                        Approve retry
                      </Button>
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => void handleReconcile(candidate, 'abandon')}
                        className="h-6 text-[10px]"
                      >
                        Abandon
                      </Button>
