  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        if (gravityClawMode) {
          const history: GCMessage[] = [...gcHistoryRef.current, { role: 'user', content: text }];
          ...
          responseText = await gcSend(history, {
            onChunk: (chunk) => {
              accumulated += chunk;
              setMessages((prev) =>
                prev.map((m) => (m.timestamp === streamKey ? { ...m, content: accumulated } : m)),
              );
            },
          });
      ...
      <Card className="flex-1 flex flex-col bg-black/40 backdrop-blur-xl border-white/10 overflow-hidden shadow-2xl">
        ...
        <div className="p-4 border-b border-white/10 flex items-center justify-between bg-white/5">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center border ${gravityClawMode ? 'bg-purple-500/20 border-purple-500/30' : 'bg-cyan-500/20 border-cyan-500/30'}`}
            >
            ...
            <div>
              ...
              <div
                className={`flex items-center gap-2 text-xs ${gravityClawMode ? 'text-purple-400' : 'text-cyan-400'}`}
              >
                <span className="relative flex h-2 w-2">
                  <span
                    className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${gravityClawMode ? 'bg-purple-400' : 'bg-cyan-400'}`}
                  />
                  <span
                    className={`relative inline-flex rounded-full h-2 w-2 ${gravityClawMode ? 'bg-purple-500' : 'bg-cyan-500'}`}
                  />
                </span>
        ...
        <MessageList
          messages={messages}
          modeLabel={modeLabel}
          gravityClawMode={gravityClawMode}
          isLoading={isLoading}
