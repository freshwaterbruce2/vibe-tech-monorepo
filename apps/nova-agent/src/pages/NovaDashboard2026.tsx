...
import {
	Activity, Brain, ChevronRight, Clock, FileText, Loader2,
	MessageSquare, PlusCircle, RefreshCw, ShieldAlert, Sparkles, Zap,
} from "lucide-react";
...
import {
	ActionButton2026, AIModelStatusPanel, GitStatusPanel, MetricCard2026,
} from "@/components/dashboard2026";
import {
	formatRelativeTime, useActivityLogger, useNovaData,
...
const NovaDashboard2026 = () => {
	...
	const {
		tasks, activities, learningEvents, focusState, agentStatus,
		context, taskStats, todayActivityCount, isLoading, error, refresh, storageHealth,
	} = useNovaData();
	...
	const handleQuickAction = async (action: string) => {
		...
		switch (action) {
			case "chat": void navigate("/chat"); break;
			case "guide": void navigate("/context-guide"); break;
			case "doc-analysis": void navigate("/document-analysis"); break;
			case "task": break;
		}
	};
	...
	if (isLoading) {
		return (
			<div className="flex items-center justify-center py-20">
				<motion.div
					style={{
						background: 'rgba(15, 12, 28, 0.8)', backdropFilter: 'blur(24px)',
						border: '1px solid rgba(176, 38, 255, 0.2)', borderRadius: '24px',
			...
			<div className="flex items-center justify-center p-4 py-20">
				<motion.div
					style={{
						...
						padding: '40px', textAlign: 'center', maxWidth: '28rem',
					}}
				...
				>
					...
					<button
						...
						style={{
							background: 'linear-gradient(135deg, #b026ff, #ff2d95)', border: 'none',
							borderRadius: '14px', padding: '14px 28px', color: 'white', fontWeight: 600,
	...
	return (
		<div style={{ color: 'white', position: 'relative' }}>
			<motion.div
				...
				style={{ opacity: 1 }} initial={{ opacity: 1 }} animate={{ opacity: 1 }}
			>
				{/* Hero Header */}
				<motion.div className="text-center space-y-6">
					<motion.div
						style={{
							...
							borderRadius: '9999px', background: 'rgba(15, 12, 28, 0.7)',
							backdropFilter: 'blur(20px)', border: '1px solid rgba(168, 85, 247, 0.2)',
						}}
					...
					>
						...
						<span style={{
							...
							border: '1px solid rgba(176, 38, 255, 0.3)', color: '#ff2d95',
				...
				<motion.div>
					<div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
						<MetricCard2026 label="Daily Activity" value={todayActivityCount} icon={Activity} color="purple" />
						<MetricCard2026 label="Active Memory" value={agentStatus?.memory_count ?? 0} icon={Brain} color="pink" />
						<MetricCard2026 label="Deep Work" value={context?.deep_work_minutes ? `${Math.floor(context.deep_work_minutes)}m` : "0m"} icon={Clock} color="cyan" />
						<MetricCard2026 label="Open Tasks" value={openTasks} icon={Zap} color="green" />
					</div>
				</motion.div>
				...
				<motion.div>
					<div style={{
						...
						backdropFilter: 'blur(24px)', border: '1px solid rgba(176, 38, 255, 0.15)',
						...
						<div className="flex flex-col md:flex-row items-center justify-between gap-6">
							...
							<motion.button
								...
								onClick={() => { void handleQuickAction("chat"); }}
				...
				<motion.div>
					...
					<div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '16px' }} className="md:grid-cols-4">
							...
							trigger={
								<motion.div
									style={{
										...
										justifyContent: 'center', gap: '12px', padding: '24px 16px',
						...
						/>
						<ActionButton2026 icon={Zap} label="Context Guide" onClick={() => { void handleQuickAction("guide"); }} />
						<ActionButton2026 icon={FileText} label="Doc Analysis" onClick={() => { void handleQuickAction("doc-analysis"); }} />
						<motion.button
							style={{
								display: 'flex', flexDirection: 'column', alignItems: 'center',
								justifyContent: 'center', gap: '12px', padding: '24px 16px',
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
					<AIModelStatusPanel agentStatus={agentStatus} context={context} />
					<GitStatusPanel context={context} />
				</div>
				...
				<div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
					<RecentActivity activities={activities} formatRelativeTime={formatRelativeTime} />
					<LearningInsights agentStatus={agentStatus} taskStats={taskStats} activities={activities} learningEvents={learningEvents} focusState={focusState} />
				</div>
			</motion.div>
		</div>
	);
