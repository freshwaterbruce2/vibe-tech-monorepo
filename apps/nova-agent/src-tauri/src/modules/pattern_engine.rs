...
impl PatternRetrievalService {
    ...
    pub fn retrieve_patterns(&self, query: &str) -> Result<Vec<SuccessPattern>, String> {
        ...
        let patterns = stmt.query_map(params![query], |row| {
            Ok(SuccessPattern {
                id: row.get(0)?,
                task_type: row.get(1)?,
    ...
    pub fn format_patterns_for_prompt(&self, patterns: &[SuccessPattern]) -> String {
        ...
        output.push_str("Based on previous successful tasks, consider these proven approaches:\n\n");

        for (i, pattern) in patterns.iter().enumerate() {
            output.push_str(&format!("{}. **{}** (Confidence: {:.0}%)\n", i + 1, pattern.description, pattern.confidence * 100.0));
            output.push_str(&format!("   - Tool Sequence: `{}`\n", pattern.tools_sequence));
        }
