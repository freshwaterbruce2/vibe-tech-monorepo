...
await app.register(fastifySwagger, {
  openapi: {
    info: {
      title: 'Monetized MCP Service API',
      description: 'API documentation for Monetized MCP Service',
      version: '0.1.0',
    },
    servers: [
      {
...
app.get('/api/emails/demo-receipt', async () => {
  ...
  const email = await buildPaymentReceiptEmail({
    ...
    paidAt: '2026-05-15',
    viewUrl,
    companyName: 'Monetized MCP Service',
...
app.post('/api/pro/rewrite', async (req, reply) => {
  ...
  return {
    ...
    rewrite: {
      recommendedCta:
        'Open the paid Monetized MCP Service workflow this week to unlock the premium action.',
    },
  };
});
...
app.get('/api/billing/demo-checkout', async (req, reply) => {
  ...
  try {
    const session = await createTenantCheckoutSession({
      ...
      lineItems: [
        {
          name: 'Monetized MCP Service Pro',
          unitAmount: 19,
        },
      ],
    });
...
app.post('/api/ai/demo-usage', async (req, reply) => {
  ...
  return {
    ...
    user: authUser,
    usage,
    summary: getAiUsage('monetized-mcp-service', 'openrouter', GENERATED_TENANT_ID),
  };
});
