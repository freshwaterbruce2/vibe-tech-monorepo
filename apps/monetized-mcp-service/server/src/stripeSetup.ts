  ...
  try {
    ...
    for (const tier of pricingTiers) {
      ...
      let product = existingProducts.data.find(
        (p: any) => p.metadata.appName === 'monetized-mcp-service' && p.metadata.plan === tier.key,
      );

      if (!product) {
        logger.info(`Creating Stripe product for ${tier.name}...`);
        product = await stripe.products.create({
          name: `Monetized MCP Service - ${tier.name}`,
          description: `Pricing plan for ${tier.name} tier`,
          metadata: {
            appName: 'monetized-mcp-service',
            plan: tier.key,
          },
        });
      }
    ...
    let customer = existingCustomers.data.find(
      (c: any) => c.metadata.appName === 'monetized-mcp-service',
    );
    ...
    if (!customer) {
      ...
      customer = await stripe.customers.create({
        email,
        name: `Monetized MCP Service Owner`,
        metadata: {
          appName: 'monetized-mcp-service',
          role: 'owner',
        },
