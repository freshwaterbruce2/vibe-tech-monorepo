...
```powershell
pnpm nx run cme-track:ship:check
```
...
PORT=5360
HOST=0.0.0.0
APP_BASE_URL=https://cme-track.your-domain.com
AUTH_SECRET=
DEMO_USER_EMAIL=
DEMO_USER_PASSWORD=
DEMO_USER_NAME=
...
```env
VITE_API_BASE_URL=https://cme-track-api.your-domain.com
VITE_POSTHOG_KEY=
VITE_POSTHOG_HOST=
...
4. Confirm `GET /api/health` returns `{"ok":true,"app":"cme-track"}`.
...
2. Set the project root to `apps/cme-track`.
3. Enable source files outside the root directory so pnpm workspace packages are available.
4. Install command: `cd ../.. && pnpm install --frozen-lockfile --filter ./apps/cme-track...`
