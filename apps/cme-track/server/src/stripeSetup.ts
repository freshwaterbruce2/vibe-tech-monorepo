  ...
  try {
    ...
    for (const tier of pricingTiers) {
      ...
      let product = existingProducts.data.find(
        (p: any) => p.metadata.appName === 'cme-track' && p.metadata.plan === tier.key,
      );

      if (!product) {
        logger.info(`Creating Stripe product for ${tier.name}...`);
        product = await stripe.products.create({
          name: `CME Track - ${tier.name}`,
    ...
    let customer = existingCustomers.data.find((c: any) => c.metadata.appName === 'cme-track');

    if (!customer) {
      logger.info(`Creating Stripe Customer for ${email}...`);
      customer = await stripe.customers.create({
        email,
        name: `CME Track Owner`,
        metadata: {
          appName: 'cme-track',
          role: 'owner',
        },
