...
export function PipelineStatus() {
  ...
  const stages = currentRun?.stages ?? BLAST_STAGES.map((s) => ({
    id: s.id,
    label: s.label,
    ...
    <div className="glass-panel panel">
      <div className="panel-header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
          ...
          <span className={`status-dot ${isRunning ? 'running' : currentRun?.status === 'completed' ? 'success' : currentRun?.status === 'failed' ? 'error' : 'idle'}`} />
          <span className="mono" style={{ fontSize: '9px', color: 'var(--text-tertiary)', letterSpacing: '0.08em' }}>
            {statusLabel}
          </span>
        </div>
      </div>
      ...
      <div className="panel-body">
        {/* BLAST Stage Bar */}
        <div className="blast-stages" style={{ marginBottom: 'var(--space-3)', justifyContent: 'center' }}>
          {stages.map((stage, i) => (
            <div key={stage.id} style={{ display: 'flex', alignItems: 'center' }}>
              <div className={`blast-stage ${stage.status === 'running' ? 'active' : ''} ${stage.status === 'success' ? 'complete' : ''} ${stage.status === 'error' ? 'error' : ''}`}>
              ...
              {stages.map((stage) => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
                    ...
                    <span style={{ fontSize: 'var(--text-sm)', fontWeight: 500 }}>{stage.label}</span>
                  </div>
              ...
              logEntries.map((entry: LogEntry, i: number) => (
                <div key={i} className="log-entry">
                  ...
                  <span className={`log-level ${entry.level}`}>
                    {entry.level === 'info' ? 'INF' : entry.level === 'done' ? 'OK' : entry.level === 'warn' ? 'WRN' : 'ERR'}
                  </span>
