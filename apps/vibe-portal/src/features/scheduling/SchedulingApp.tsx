...
export function SchedulingApp({ embedded = false }: { embedded?: boolean }) {
  ...
  return <ClinicDashboard apiBase={apiBase} embedded={embedded} />;
}

function ClinicDashboard({ apiBase, embedded }: { apiBase: string; embedded: boolean }) {
  ...
  const content = (
    <>
      <header className="scheduling-hero">
        <div>
  ...
  return embedded ? <section id="scheduling" className="portal-section scheduling-section">{content}</section> : <main className="portal-standalone scheduling-section">{content}</main>;
}
