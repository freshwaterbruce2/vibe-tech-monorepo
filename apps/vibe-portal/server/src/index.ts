  ...
  try {
    ...
    if (!existingUser) {
      ...
      if (!existingCompany) {
        ...
        `).run(companyId, 'Vibe-Tech Owner Company', defaultPlan);
      }
      
      db.prepare(`
        INSERT INTO users (id, email, password_hash, first_name, last_name, company_id, role, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
      `).run(userId, email, 'no-password-hash-needed-for-demo', 'Vibe-Tech', 'Owner', companyId, 'owner');
      ...
      if (email) {
        try {
          ...
          if (fs.existsSync(manifestPath)) {
            ...
            const addedMrr = plan === 'enterprise' ? 29900 : plan === 'starter' ? 2900 : 7900;
...
app.post('/api/ai/demo-usage', async (req, reply) => {
  ...
  return {
    ...
    user: authUser,
    usage,
    summary: getAiUsage('client-portal', 'openrouter', GENERATED_TENANT_ID),
  };
});
