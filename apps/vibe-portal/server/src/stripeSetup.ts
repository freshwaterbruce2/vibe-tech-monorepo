  ...
  try {
    ...
    const pricingTiers = [
      {
        ...
        name: 'Free',
        price: 0,
        features: ['project.management', 'file.sharing', 'communication.channels'],
      },
      {
        key: 'starter',
        name: 'Starter',
    ...
    for (const tier of pricingTiers) {
      ...
      let product = existingProducts.data.find(
        (p: any) => p.metadata.appName === 'client-portal' && p.metadata.plan === tier.key,
      );

      if (!product) {
        logger.info(`Creating Stripe product for ${tier.name}...`);
        product = await stripe.products.create({
          name: `Vibe-Tech - ${tier.name}`,
    ...
    let customer = existingCustomers.data.find((c: any) => c.metadata.appName === 'client-portal');

    if (!customer) {
      logger.info(`Creating Stripe Customer for ${email}...`);
      customer = await stripe.customers.create({
        email,
        name: `Vibe-Tech Owner`,
        metadata: {
          appName: 'client-portal',
          role: 'owner',
        },
