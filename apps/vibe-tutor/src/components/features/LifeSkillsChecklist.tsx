    ...
    <div className="glass-card p-6 rounded-2xl border border-white/10">
        ...
        {filteredSkills.map((skill) => {
          ...
          return (
            ...
            >
              ...
              <skill.icon
                className={`w-4 h-4 flex-shrink-0 ${isCompleted ? 'text-green-400' : 'text-gray-400'}`}
              />
              <span
                className={`text-sm flex-1 text-left ${isCompleted ? 'text-green-300 line-through' : 'text-white'}`}
              >
                {skill.task}
              </span>
            </button>
      ...
      {completionRate === 100 && (
        <div className="mt-4 p-4 bg-green-500/10 border border-green-500/30 rounded-lg text-center">
          <p className="text-green-400 font-semibold">Amazing! All tasks complete today! 🎉</p>
