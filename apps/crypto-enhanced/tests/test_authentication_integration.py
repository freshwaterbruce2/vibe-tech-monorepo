...
def test_kraken_authentication():
    ...
    try:
        ...
        pytest.skip(f"asyncio unavailable in this environment: {exc}")

    try:
        from kraken_client import KrakenClient
    except (ImportError, OSError) as exc:
        pytest.skip(f"Kraken client unavailable in this environment: {exc}")

    print(f"\nAPI Key loaded: {config.kraken_api_key[:8]}...")

    async def exercise_authentication():
    ...
    asyncio.run(exercise_authentication())
