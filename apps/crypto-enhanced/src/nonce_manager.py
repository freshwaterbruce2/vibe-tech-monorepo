...
class NonceManager:
    ...
    def _load_nonce(self) -> int:
        ...
        if self.KRAKEN_REMEMBERED_NONCE == 0:
            initial_nonce = TimestampUtils.now_ns()  # Nanoseconds (standard for high-frequency)
            logger.info(f"Starting with fresh nonce for new keys: {initial_nonce}")
        else:
            # Continue from known high value + buffer, but ensure we are ahead of time if time > stored
            current_ns = TimestampUtils.now_ns()
            initial_nonce = max(self.KRAKEN_REMEMBERED_NONCE + 10000, current_ns)
