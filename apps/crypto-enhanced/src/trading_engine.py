...
class DataPruner:
    ...
    def __init__(
        self, max_trades: int = 100, max_tickers: int = 50, ttl_hours: int = 24
    ):
...
class ExposureCache:
    ...
    def get_total_exposure(self, positions: Dict, calculate_func: Callable) -> Decimal:
        ...
        if (
            self._total_exposure_cache is None
            or self._total_exposure_timestamp is None
            or (now - self._total_exposure_timestamp).total_seconds() > self._ttl
        ):
    ...
    def can_attempt(self) -> bool:
        ...
        if self.last_failure_time:
            ...
            if elapsed > self.timeout:
                logger.info(
                    "CircuitBreaker: Auto-recovery timeout elapsed, closing circuit"
                )
                self.is_open = False
                self.failures = 0
                return True
...
class TradingEngine:
    ...
    def __init__(self, kraken_client, websocket_manager, database, config):
        ...
        # OHLC data store for ATR calculation (with timestamp safety)
        self.ohlc_store = {}  # symbol -> list of price ticks
        self.atr_cache = {}  # symbol -> current ATR value
        self.last_atr_update = {}  # symbol -> timestamp of last ATR calc
    ...
    async def run(self):
        ...
        while self.running:
            try:
                ...
                # Reactive Wait: Wait for new data or timeout (heartbeat)
                try:
                    await asyncio.wait_for(
                        self.data_updated.wait(),
                        timeout=self.config.engine_loop_interval,
                    )
                    self.data_updated.clear()
                except asyncio.TimeoutError:
                    pass  # Continue loop on timeout (heartbeat)

            except Exception as e:
                log_error(e, "trading_loop")
                logger.error(
                    f"Trading engine error (iteration {loop_iteration}): {e}",
                    exc_info=True,
                )
    ...
    def _monitor_memory_usage(self):
        ...
        try:
            ...
            # Count items
            ticker_count = len(
                [k for k in self.market_data.keys() if k.endswith("_ticker")]
            )
            trade_count = len(
                [k for k in self.market_data.keys() if k.endswith("_trades")]
            )
            book_count = len(
                [k for k in self.market_data.keys() if k.endswith("_book")]
            )
    ...
    async def _initialize_strategies(self):
        ...
        try:
            ...
            logger.info(
                f"Initialized {len(self.strategies)} strategies: {strategy_names}"
            )
        ...
        for position_id, position in positions_snapshot.items():
            try:
                ...
                if not position.get("stop_loss") and entry_price > 0:
                    ...
                    logger.info(
                        f"[AUTO-SET] Stop-loss for {position_id}: ${stop_loss:.4f} (-1.5% from ${entry_price:.4f})"
                    )
                ...
                if not position.get("take_profit") and entry_price > 0:
                    ...
                    logger.info(
                        f"[AUTO-SET] Take-profit for {position_id}: ${take_profit:.4f} (+1.5% from ${entry_price:.4f})"
                    )
                ...
                if position.get("stop_loss"):
                    if self._should_stop_loss(position, current_price):
                        await self.close_position(
                            position_id, reason="Stop loss triggered"
                        )
                ...
                if position.get("take_profit"):
                    if self._should_take_profit(position, current_price):
                        await self.close_position(
                            position_id, reason="Take profit triggered"
                        )
    ...
    async def _get_current_price(self, pair: str) -> Decimal:
        ...
        if ticker_key in self.market_data:
            ...
            if "last" in ticker:
                ...
                close_price = (
                    ticker["c"][0] if isinstance(ticker["c"], list) else ticker["c"]
                )
    ...
    def get_capital_summary(self) -> Dict:
        """
        Get clear breakdown of deployed vs available capital

        Returns dict with:
        - total_usd: Total USD in account
        - deployed_usd: USD value of open positions
        - available_usd: USD available for new trades
        - xlm_holdings: Total XLM held
        - xlm_value_usd: USD value of XLM holdings
        """
        balance = self.market_data.get("balance", {})
        usd_balance = float(balance.get("ZUSD", balance.get("USD", 0)))
        xlm_balance = float(balance.get("XXLM", balance.get("XLM", 0)))

        # Calculate deployed capital (value of open positions)
        deployed_usd = 0.0
        for position in self.open_positions.values():
            volume = float(position.get("volume", 0))
            entry_price = float(position.get("entry_price", 0))
            deployed_usd += volume * entry_price

        # Get current XLM price for valuation
        xlm_price = self._get_xlm_price() or 0.0
        xlm_value_usd = xlm_balance * xlm_price

        # Available is total minus deployed
        available_usd = usd_balance - deployed_usd

        return {
            "total_usd": usd_balance,
            "deployed_usd": deployed_usd,
            "available_usd": max(0, available_usd),  # Never negative
            "xlm_holdings": xlm_balance,
            "xlm_value_usd": xlm_value_usd,
            "total_portfolio_usd": usd_balance + xlm_value_usd,
            "deployment_ratio": (deployed_usd / usd_balance * 100)
            if usd_balance > 0
            else 0,
        }
    ...
    def calculate_position_pnl(self, position: Dict, current_price: float) -> Dict:
        ...
        # Guard against division by zero for edge cases where
        # volume or entry_price are missing or zero.
        notional = volume * entry_price
        if notional > 0:
            pnl_percentage = (net_pnl / notional) * 100
        else:
    ...
    async def _on_execution_update(self, data: Dict):
        ...
        if "data" in data and isinstance(data["data"], list):
            for exec_data in data["data"]:
                ...
                if (
                    exec_data.get("order_status") == "filled"
                    or exec_data.get("exec_type") == "filled"
                ):
                    ...
                    if order_id and side == "buy":
                        ...
                        self.open_positions[order_id] = {
                            ...
                            "side": "buy",
                            "volume": exec_data.get(
                                "cum_qty", exec_data.get("last_qty", 0)
                            ),
                            "entry_price": exec_data.get(
                                "avg_price", exec_data.get("last_price", 0)
                            ),
                            "stop_loss": pending_order.get("stop_loss"),
                            "take_profit": pending_order.get("take_profit"),
                            "created_at": exec_data.get(
    ...
    async def place_order(
        ...
        if "XLM" in pair and risk_check_price:
            ...
            if side == OrderSide.BUY and self.last_xlm_trade_time:
                ...
                if time_since < cooldown_period:
                    ...
                    logger.warning(
                        f"XLM cooldown active: {remaining:.0f} seconds remaining"
                    )
                    return {
                        "error": f"XLM cooldown active, wait {remaining:.0f} seconds"
                    }

            if not stop_loss:
                # Calculate stop-loss from percentage (xlm_stop_loss_pct = 0.015 = 1.5%)
                stop_loss_price = risk_check_price * (1 - self.config.xlm_stop_loss_pct)
                stop_loss = str(stop_loss_price)
                logger.info(
                    f"Setting XLM stop-loss at ${stop_loss} (-{self.config.xlm_stop_loss_pct * 100:.1f}% from ${risk_check_price:.4f})"
                )
            if not take_profit:
                # Calculate take-profit from percentage (xlm_take_profit_pct = 0.015 = 1.5%)
                take_profit_price = risk_check_price * (
                    1 + self.config.xlm_take_profit_pct
                )
                take_profit = str(take_profit_price)
                logger.info(
                    f"Setting XLM take-profit at ${take_profit} (+{self.config.xlm_take_profit_pct * 100:.1f}% from ${risk_check_price:.4f})"
                )
        ...
        if "XLM" in pair:
            ...
            if (
                risk_check_price < self.config.xlm_price_range_min
                or risk_check_price > self.config.xlm_price_range_max
            ):
                logger.warning(
        ...
        if limit_price:
            order_cost = self.calculate_order_cost(
                order_qty, limit_price, side, order_type
            )
        else:
            current_ticker = self.market_data.get(f"{symbol}_ticker", {})
            est_price = float(
                current_ticker.get("ask" if side == OrderSide.BUY else "bid", 0)
            )
            if est_price > 0:
                order_cost = self.calculate_order_cost(
                    order_qty, est_price, side, order_type
                )
            else:
        ...
        async def ws_place_order():
            ...
            logger.info(
                f"WebSocket order placed: {symbol} {side.value} {volume} @ {price}"
            )
            return result
    ...
    async def close_position(self, position_id: str, reason: str = ""):
        ...
        try:
            ...
            logger.error(
                f"[CLOSE POSITION] EXCEPTION closing {position_id}: {e}", exc_info=True
            )
            await self.db.log_order(
                {
                    "position_id": position_id,
...
# ============================================================================
# RISK MANAGER - ENHANCED WITH ATR-BASED DYNAMIC SIZING
# ============================================================================


class RiskManager:
    """
    Hybrid Risk Manager that wraps EnhancedRiskManager for ATR-based sizing
    while maintaining backward compatibility with existing code

    Features:
    - ATR-based dynamic position sizing (when enabled)
    - Market regime detection (calm, trending, choppy, high_volatility)
    - Adaptive stop-loss distances
    - Fallback to basic risk checks if enhanced mode disabled

    IMPROVEMENTS over basic version:
    - Volatility-responsive position sizing
    ...
    def __init__(self, config):
        ...
        # Initialize EnhancedRiskManager if enabled
        if getattr(config, "use_enhanced_risk", False):
            try:
                from risk.enhanced_risk_manager import EnhancedRiskManager

                self.enhanced = EnhancedRiskManager(
                    base_kelly_fraction=getattr(config, "base_kelly_fraction", 0.02),
                    max_leverage=getattr(config, "max_leverage", 3.0),
                    min_position_fraction=getattr(
                        config, "min_position_fraction", 0.01
                    ),
                    max_position_fraction=getattr(
                        config, "max_position_fraction", 0.25
                    ),
                    atr_lookback=getattr(config, "atr_lookback", 14),
                    regime_lookback=getattr(config, "regime_lookback", 50),
                )

                logger.info(
                    "EnhancedRiskManager initialized: "
                    f"kelly={config.base_kelly_fraction}, "
                    f"max_frac={config.max_position_fraction}, "
                    f"atr_period={config.atr_lookback}"
                )
            except ImportError as e:
                logger.warning(
                    f"Could not import EnhancedRiskManager: {e}. Using basic risk manager."
                )
                self.enhanced = None
        else:
            self.enhanced = None
            logger.info("Using basic risk manager (enhanced mode disabled)")
    ...
    def get_position_size_fraction(
        ...
        """
        Get dynamic position size fraction based on volatility (ATR)

        If enhanced mode is disabled or ATR not available, returns fixed fraction
        based on max_position_size / balance
        ...
        """
        if self.enhanced and atr is not None and atr > 0:
            # Use ATR-based dynamic sizing
            fraction = self.enhanced.position_size_fraction(
                price=price, atr=atr, balance=balance, symbol=symbol
            )
            logger.debug(
                f"Dynamic position sizing: {symbol} price=${price:.4f}, "
                f"atr={atr:.4f}, fraction={fraction:.4f}"
            )
            return fraction
        else:
            # Fallback to fixed sizing based on config
            fixed_fraction = (
                float(self.max_position_size) / balance if balance > 0 else 0.01
            )
            fixed_fraction = min(fixed_fraction, 0.25)  # Cap at 25%
    ...
    def detect_regime(self, atr: float, price: float, symbol: str = "XLM/USD") -> str:
        ...
        Returns:
            Regime string: "calm", "trending", "choppy", "high_volatility", or "unknown"
        """
        if self.enhanced and atr > 0 and price > 0:
            return self.enhanced.detect_regime(atr=atr, price=price, symbol=symbol)
        return "unknown"

    def calculate_adaptive_stop_loss(
        self, price: float, atr: float, regime: Optional[str] = None
    ) -> float:
        """
        ...
        Args:
            price: Current price
            atr: Current ATR
            regime: Market regime (optional, will detect if not provided)

        Returns:
            Stop-loss distance in price units
        """
        if self.enhanced and atr > 0:
            if regime is None:
                regime = self.enhanced.detect_regime(
                    atr=atr, price=price, symbol="XLM/USD"
                )

            return self.enhanced.calculate_stop_loss_distance(
                price=price, atr=atr, regime=regime
            )
        else:
            # Fallback to fixed percentage (1.5%)
            return price * 0.015

    def get_regime_stats(self, symbol: str = "XLM/USD") -> dict:
        """Get current regime statistics for monitoring"""
        if self.enhanced:
            return self.enhanced.get_regime_stats(symbol=symbol)
    ...
    async def approve_order(
        ...
        This method supports both the newer explicit signature:
            approve_order(pair, side, volume, positions, price, balance)
        and the older, test-oriented signature:
            approve_order(pair, volume, positions, price)
        where side defaults to BUY.
        """
        # ------------------------------------------------------------------
        # Backward-compatibility shim for older call style used in tests:
        #   approve_order('BTC/USD', '0.1', positions, 250.0)
        # In that case:
        #   - side is actually the volume string
        #   - volume is the positions dict
        #   - positions is the price
        # ------------------------------------------------------------------
        if (
            isinstance(side, str)
            and not isinstance(volume, str)
            and isinstance(positions, (int, float, Decimal))
        ):
            # Old positional style: (pair, volume_str, positions_dict, price_float)
            original_volume = side
            original_positions = volume
            original_price = positions
        ...
        # If side is still None, default to BUY (used by some keyword-based tests)
        if side is None:
            side = OrderSide.BUY

        # Ensure we have required fields at this point
        if volume is None or positions is None:
            logger.error(
                f"RiskManager.approve_order called with insufficient parameters: "
                f"pair={pair}, side={side}, volume={volume}, positions={positions}"
            )
            return False
        ...
        if side == OrderSide.BUY and self.current_balance:
            ...
            # Check minimum balance requirement (total balance check)
            if usd_balance < self.config.min_balance_required:
                logger.error(
                    f"Insufficient total balance: ${usd_balance:.2f} < "
                    f"${self.config.min_balance_required} minimum"
                )
                return False

            # CRITICAL: Calculate AVAILABLE capital (not deployed in positions)
            # This prevents using already-deployed capital for new trades
            current_exposure = float(self._calculate_total_exposure(positions))
            available_capital = usd_balance - current_exposure

            # Check if we have enough AVAILABLE capital for this order plus buffer
            required_balance = float(position_value) * 1.1  # 10% buffer
            if available_capital < required_balance:
                logger.warning(
                    f"Insufficient AVAILABLE capital: ${available_capital:.2f} available "
                    f"(${usd_balance:.2f} total - ${current_exposure:.2f} deployed) < "
                    f"${required_balance:.2f} required for order"
                )
                return False

            # Warning if available capital is low
            if available_capital < self.config.min_balance_alert:
                logger.warning(
                    f"Low available capital: ${available_capital:.2f} "
                    f"(${usd_balance:.2f} total - ${current_exposure:.2f} deployed)"
                )
        ...
        if (
            side == OrderSide.BUY
            and total_exposure + position_value > self.max_total_exposure
        ):
            logger.warning(
