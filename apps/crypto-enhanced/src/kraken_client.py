...
class KrakenClient:
    ...
    def __init__(self, config, use_secondary_key: bool = False):
        ...
        # Store in cache dir (D:\) to satisfy data quarantine
        nonce_dir = Path(config.cache_dir)
        nonce_dir.mkdir(parents=True, exist_ok=True)
        nonce_file = nonce_dir / f"nonce_state_{self.key_label}.json"
        self.nonce_manager = NonceManager(storage_path=str(nonce_file))
