...
class RSIMeanReversionStrategy(Strategy):
    ...
    def __init__(self, engine, config):
        ...
        self.min_profit_target = strategy_config.get('min_profit_target', 0.015)  # 1.5% to cover fees + profit
        # IMPORTANT: Reduced from 100 to 50 XLM for safer position sizing
        # 50 XLM * $0.35 = $17.50, approximately 18% of $100 balance
        self.max_accumulation = strategy_config.get('max_position_accumulation_xlm', 50)
    ...
    async def evaluate(self) -> Optional[Dict]:
        ...
        try:
            ...
            # Check for overbought condition (sell signal) - only if we have positions
                ...
                # CRITICAL FIX: Sell FULL accumulated position, not fixed USD
                # This ensures we close the entire position when exit signal triggers
                sell_volume_xlm = self.accumulated_xlm
                if sell_volume_xlm < self.config.xlm_min_order_size:
                    logger.warning(f"{self.name}: Accumulated XLM ({sell_volume_xlm:.2f}) below minimum order size")
                    return None

                sell_value_usd = sell_volume_xlm * current_price
                logger.info(f"{self.name}: RSI overbought signal - RSI={rsi:.2f}, Price=${current_price:.4f}, Selling ALL {sell_volume_xlm:.2f} XLM (${sell_value_usd:.2f})")
                result = await self.place_trade("sell", sell_value_usd, "limit", current_price * 0.999)

                # Reset accumulation tracking if trade was placed
                if result and 'error' not in result:
                    self.accumulated_xlm = 0
                    self.entry_count = 0
                    logger.info(f"{self.name}: Full position closed - accumulation reset to 0")
...
class RangeTradingStrategy(Strategy):
    ...
    def __init__(self, engine, config):
        ...
        # IMPORTANT: Reduced from 80 to 40 XLM for safer position sizing
        # 40 XLM * $0.35 = $14, approximately 14% of $100 balance
        self.max_accumulation = strategy_config.get('max_position_accumulation_xlm', 40)
    ...
    async def evaluate(self) -> Optional[Dict]:
        ...
        try:
            ...
            if current_price >= sell_threshold and len(self.engine.positions) > 0:
                ...
                sell_value_usd = sell_volume_xlm * current_price
                logger.info(f"{self.name}: Range sell signal - Price ${current_price:.4f} near resistance ${self.resistance_level:.4f}, Selling ALL {sell_volume_xlm:.2f} XLM (${sell_value_usd:.2f})")
                result = await self.place_trade("sell", sell_value_usd, "limit", current_price * 0.9995)
        ...
        try:
            ...
            if len(last_3) == 3:
                ...
                if is_trending_up and current_price < ask * 1.002:  # Buy on uptrend
                    ...
                    # CRITICAL FIX: Get actual XLM from positions, not fixed USD
                    total_xlm = sum(
                        float(pos.get('volume', 0))
                        for pos in self.engine.positions.values()
                        if pos.get('pair') == 'XLM/USD'
                    )
                    if total_xlm < self.config.xlm_min_order_size:
                        logger.warning(f"{self.name}: Position XLM ({total_xlm:.2f}) below minimum order size")
                        return None

                    sell_value_usd = total_xlm * current_price
                    logger.info(f"{self.name}: Scalp sell signal - momentum down, price ${current_price:.4f}, Selling {total_xlm:.2f} XLM")
                    self.last_hour_trades.append(current_time)
                    return await self.place_trade("sell", sell_value_usd, "limit", ask * 0.9999)
...
class StrategyManager:
    """Manages multiple strategies and their execution with coordination"""
    ...
    def __init__(self, engine, config):
        ...
        self.strategies = []
        self.enabled = True
        # Strategy coordination lock to prevent race conditions
        self._evaluation_lock = asyncio.Lock()
        # Track pending orders across strategies to prevent exceeding limits
        self._pending_strategy_orders = 0
    ...
    async def evaluate_all(self):
        """Evaluate all enabled strategies with coordination to prevent exceeding limits"""
        if not self.enabled or not self.strategies:
            return

        # CRITICAL: Use lock to prevent race conditions between strategies
        async with self._evaluation_lock:
            # Get current position count BEFORE any strategy runs
            ...
            for strategy in self.strategies:
                ...
                # CRITICAL: Check if we can open new positions BEFORE evaluating
                # Skip BUY-oriented evaluation if at max positions
                if current_positions >= max_positions:
                    logger.debug(f"{strategy.name}: Skipping - at max positions ({current_positions}/{max_positions})")
                    # Still allow sell evaluations by checking if strategy has positions
                    xlm_positions = [p for p in self.engine.positions.values() if p.get('pair') == 'XLM/USD']
                    if not xlm_positions:
                        continue
                    # Strategy may still sell, so continue with limited evaluation

                try:
                    result = await strategy.evaluate()
                    # If a BUY order was placed, increment position count for next strategy
                    if result and 'error' not in result and result.get('order_id'):
                        # Check if it was a buy order (increases positions)
                        if strategy.last_trade_time and hasattr(strategy, 'accumulated_xlm'):
                            if strategy.accumulated_xlm > 0:
                                current_positions += 1
                                logger.info(f"{strategy.name}: Order placed, positions now {current_positions}/{max_positions}")
                except Exception as e:
