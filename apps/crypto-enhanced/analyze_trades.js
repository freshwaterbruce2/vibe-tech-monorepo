...
try {
	...
	if (tradeCount.count > 0) {
		...
		recentTrades.forEach((trade, i) => {
			...
			console.log(
				`   Quantity: ${trade.quantity} | Price: $${trade.price?.toFixed(6)}`,
			);
			console.log(
				`   Value: $${(trade.quantity * trade.price).toFixed(2)} | Fee: $${trade.fee?.toFixed(4)}`,
			);
		...
		console.log(
			`Average Fee per Trade: $${(allFees / allTrades.length).toFixed(4)}`,
		);
	}
	...
	try {
		const positions = db
			.prepare("SELECT * FROM positions ORDER BY timestamp DESC")
			.all();
		...
		if (positions.length === 0) {
			...
			if (openPos.length > 0) {
				...
				openPos.forEach((pos) => {
					console.log(
						`  ${pos.symbol}: ${pos.quantity} @ $${pos.entry_price?.toFixed(6)}`,
					);
				});
			}
		}
	...
	try {
		const orders = db
			.prepare("SELECT * FROM orders ORDER BY timestamp DESC LIMIT 10")
			.all();
		...
		if (orders.length > 0) {
			...
			orders.forEach((order) => {
				console.log(
					`  ${order.symbol} ${order.side?.toUpperCase()}: ${order.quantity} @ $${order.price?.toFixed(6)} - Status: ${order.status}`,
				);
			});
		} else {
			console.log("\nNo orders found");
