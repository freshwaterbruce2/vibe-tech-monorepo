 ...
 *   ----------------------------  -----------------------  ----------------------------------------
 *   Workspace Stability / drift   GET  /workspace          scripts/workspace-health.ps1 + scanWorkspace
 *   Database health / "locks"     GET  /databases          scripts/database-health.ps1
 *   D:\ drive health              GET  /d-drive            scripts/d-drive-health.ps1
 *   AI-training crash (log)       GET  /logs/errors        D:/learning-system/logs/*.log
 *   "Fix & Align" (cleanup)       POST /cleanup            scripts/cleanup-stale-artifacts.ps1 [-DryRun]
 *   "Unlock DB" / maintenance     POST /maintenance        learning-system/scripts/run_maintenance.py
 *
...
/** Run a PowerShell script under V:\monorepo\scripts. */
function runPs(script: string, args = ''): Promise<RunResult> {
...
healthRouter.get('/workspace', async (_req: Request, res: Response) => {
  const [health, scan] = await Promise.all([runPs('workspace-health.ps1'), scanWorkspace(WORKSPACE_ROOT)]);
  res.json({ health, packages: { count: scan.count, includeGlobs: scan.includeGlobs, items: scan.packages } });
});
...
healthRouter.post('/maintenance', async (req: Request, res: Response) => {
  if (req.body?.confirm !== true) {
    res.status(412).json({ success: false, error: 'maintenance is destructive; resend with { "confirm": true }' });
    return;
  }
  const { retention, vacuum, backup } = req.body as { retention?: boolean; vacuum?: boolean; backup?: boolean };
  const flags = [retention && '--retention', vacuum && '--vacuum', backup && '--backup'].filter(Boolean).join(' ');
