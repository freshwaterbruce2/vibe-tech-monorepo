...
export class RAGCache {
    ...
    this.db
      .prepare(`
        INSERT OR REPLACE INTO query_cache
          (key, query_text, results, created_at, ttl_ms, hit_count, file_paths)
        VALUES (?, ?, ?, ?, ?, 0, ?)
      `)
      .run(
        key,
  ...
  invalidateByPaths(filePaths: string[]): number {
    const allEntries = this.db
      ...
      .all() as { key: string; file_paths: string }[];
    const changedSet = new Set(filePaths);
    const keysToDelete = allEntries.filter((e) => {
      try {
        return (JSON.parse(e.file_paths) as string[]).some((p) => changedSet.has(p));
      } catch {
        return false;
      }
    }).map((e) => e.key);
