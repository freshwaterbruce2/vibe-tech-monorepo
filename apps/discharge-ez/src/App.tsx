...
export function App() {
  ...
  async function handleSimplify() {
    ...
    try {
      ...
      const data = (await response.json()) as { simplifiedText?: string; error?: string };
      if (!response.ok) {
        throw new Error(data.error ?? 'Failed to simplify text');
      }
      setSimplifiedText(data.simplifiedText ?? '');
    } catch (e) {
      setSimplifyError(e instanceof Error ? e.message : String(e));
    } finally {
  ...
  return (
    <main>
      ...
      <section className="page" style={{ padding: '24px', maxWidth: '1120px', margin: '0 auto' }}>
        <article className="grid-card" style={{ width: '100%' }}>
          <div className="eyebrow">AI Simplifier</div>
          <h2>Discharge Instruction Simplifier</h2>
          <p>
            Paste complex clinical discharge instructions below to translate them to a 5th-grade reading level.
          </p>
          <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <textarea
              value={clinicalText}
              onChange={(e) => setClinicalText(e.target.value)}
              placeholder="Paste patient clinical discharge instructions here..."
              rows={6}
              style={{
                width: '100%',
                padding: '12px',
            ...
            <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
              ...
              <button
                ...
                disabled={simplifyLoading || !clinicalText}
                style={{
                  alignSelf: 'flex-end',
                  padding: '10px 20px',
                  borderRadius: '6px',
            ...
            {simplifiedText && (
              ...
              >
                <div style={{ fontSize: '12px', color: '#67e8f9', textTransform: 'uppercase', marginBottom: '8px', fontWeight: 'bold' }}>
                  Simplified Instructions ({targetLanguage})
                </div>
                <div style={{ whiteSpace: 'pre-wrap', color: '#f5f7fb', fontSize: '14px', lineHeight: '1.6' }}>
                  {simplifiedText}
                </div>
              </div>
            )}
