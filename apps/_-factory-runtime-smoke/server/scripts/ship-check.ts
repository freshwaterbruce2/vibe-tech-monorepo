...
const authSecret = '_-factory-runtime-smoke-local-auth-secret-12345';
const operatorEmail = 'owner@example.com';
const operatorPassword = 'change-this-password';
const operatorName = 'FactoryRuntimeSmoke Owner';
...
async function checkEnvPresence(keys: string[], id: string, successDetail: string): Promise<void> {
  ...
  results.push({
    ...
    ok: missing.length === 0,
    detail:
      missing.length === 0 ? successDetail : `Missing required env keys: ${missing.join(', ')}`,
  });
}
