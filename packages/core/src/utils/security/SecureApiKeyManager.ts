...
export class SecureApiKeyManager {
  ...
  private constructor(logger?: Logger) {
    ...
    // Check if running in Electron (renderer) or Tauri shim
    ...
    } else if (rendererWindow.electron?.store) {
      this.isElectron = true;
      // Adapt window.electron.store (Tauri) to ElectronStorageAPI
      this.electronStorage = {
        get: async (key: string) => {
          try {
            const val = await rendererWindow.electron!.store!.get(key);
            let parsed = val;
