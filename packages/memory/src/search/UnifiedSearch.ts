...
export class UnifiedSearch {
  ...
  async search(query: string, options?: UnifiedSearchOptions): Promise<UnifiedSearchResult[]> {
    ...
    // Token budget: if set, allocate per-source slices and trim each source's
    // results in score order until its slice is full. Runs BEFORE RRF so that
    // truncation respects the caller's intra-source ranking, not the fused one.
    const budgeted = options?.tokenBudget
      ? this.applyTokenBudget(allResults, options.tokenBudget, options.sourceWeights, sources)
      : allResults;
