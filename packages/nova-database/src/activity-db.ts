...
interface FileEventRow {
  id: number;
  path: string;
  event_type: FileEvent['eventType'];
  timestamp: number;
  project: FileEvent['project'];
  old_path: FileEvent['oldPath'];
}

interface GitEventRow {
  id: number;
  repo_path: string;
  event_type: GitEvent['eventType'];
  branch: GitEvent['branch'];
  commit_hash: GitEvent['commitHash'];
...
interface ProcessEventRow {
  ...
  port: ProcessEvent['port'];
  command_line: ProcessEvent['commandLine'];
  project: ProcessEvent['project'];
  timestamp: number;
}
...
export class ActivityDatabase {
  ...
  constructor(dbPath: string = DB_PATH) {
    ...
    assertSafeDatabasePath(resolvedPath, 'activity');
  ...
  getFileEvents(filter: ActivityFilter = {}): FileEvent[] {
    ...
    const rows = stmt.all(...params) as FileEventRow[];
  ...
  getGitEvents(filter: ActivityFilter = {}): GitEvent[] {
    ...
    const stmt = this.db.prepare(query);
    const rows = stmt.all(...params) as GitEventRow[];
  ...
  getProcessEvents(filter: ActivityFilter = {}): ProcessEvent[] {
    ...
    const stmt = this.db.prepare(query);
    const rows = stmt.all(...params) as ProcessEventRow[];
