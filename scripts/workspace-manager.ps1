...
function Show-WorkspaceStatus {
    ...
    Write-Host "  Location: apps/crypto-enhanced" -ForegroundColor White
    $venvExists = Test-Path "apps/crypto-enhanced/.venv"
    ...
    if (Test-Path "apps/vibe-tech-lovable") {
        ...
        $frontendDeps = Test-Path "apps/vibe-tech-lovable/node_modules"
        $backendDeps = Test-Path "apps/vibe-tech-lovable/backend/node_modules"
...
function Install-WorkspaceProjects {
    ...
    if ($All -or $Project -eq "crypto") {
        ...
        Push-Location "apps/crypto-enhanced"
        try {
            python -m venv .venv
...
function Start-DevEnvironment {
    ...
    if ($Project -eq "root" -or -not $Project) {
        if (Test-PortAvailable 5173) {
            ...
            Start-Process "pnpm" -ArgumentList "run", "dev" -WorkingDirectory $PWD | Out-Null
        }
        else {
            Write-Host "[WARN] Root dev server already running on port 5173" -ForegroundColor Yellow
        }
    }

    if ($Project -eq "crypto") {
        if (Test-PortAvailable 8000) {
            $cryptoDir = Join-Path $PWD "apps/crypto-enhanced"
            if (Test-Path "$cryptoDir\.venv\Scripts\python.exe") {
                Write-Host "Starting crypto trading system..." -ForegroundColor Yellow
                Start-Process "$cryptoDir\.venv\Scripts\python.exe" -ArgumentList "start_live_trading.py" -WorkingDirectory $cryptoDir | Out-Null
            }
            else {
                Write-Host "[WARN] Crypto venv missing at apps/crypto-enhanced/.venv" -ForegroundColor Yellow
            }
        }
        else {
            Write-Host "[WARN] Crypto service already running on port 8000" -ForegroundColor Yellow
        }
    }

    if ($Project -eq "vibe-lovable") {
        $vibeDir = Join-Path $PWD "apps/vibe-tech-lovable"
        if (-not (Test-Path $vibeDir)) {
            Write-Host "[WARN] apps/vibe-tech-lovable not found" -ForegroundColor Yellow
            return
        }

        if (Test-PortAvailable 8080) {
            Write-Host "Starting Vibe-Tech Lovable frontend..." -ForegroundColor Yellow
            Start-Process "pnpm" -ArgumentList "run", "dev" -WorkingDirectory $vibeDir | Out-Null
        }
        else {
            Write-Host "[WARN] Vibe frontend already running on port 8080" -ForegroundColor Yellow
        }

        $backendDir = Join-Path $vibeDir "backend"
        if (Test-Path $backendDir) {
            if (Test-PortAvailable 9001) {
                Write-Host "Starting Vibe-Tech Lovable backend..." -ForegroundColor Yellow
                Start-Process "pnpm" -ArgumentList "run", "dev" -WorkingDirectory $backendDir | Out-Null
            }
            else {
                Write-Host "[WARN] Vibe backend already running on port 9001" -ForegroundColor Yellow
            }
        }
    }
}
...
function Test-WorkspaceHealth {
    ...
    if (Test-Path "apps/crypto-enhanced/.venv") {
...
function Start-ParallelProject {
    ...
    switch ($ProjectName) {
        ...
        "vibe-lovable" {
            $vibePath = "apps/vibe-tech-lovable"
            if (-not (Test-Path $vibePath)) {
                Write-Host "[WARN] $vibePath not found" -ForegroundColor Yellow
                return $null
            }
            ...
            if (Test-PortAvailable 8080) {
                ...
                Write-Host "[OK] vibe-lovable-frontend started (Job ID: $($feJob.Id))" -ForegroundColor Green
            }
            else {
                Write-Host "[WARN] Port 8080 already in use for vibe-lovable frontend" -ForegroundColor Yellow
            }
            ...
            if (Test-PortAvailable 9001) {
                ...
                Write-Host "[OK] vibe-lovable-backend started (Job ID: $($beJob.Id))" -ForegroundColor Green
            }
            else {
                Write-Host "[WARN] Port 9001 already in use for vibe-lovable backend" -ForegroundColor Yellow
            }
        ...
        "memory-bank" {
            ...
            if (Test-Path "$memoryPath\monitoring_service.py") {
                ...
                Write-Host "[WARN] Memory bank service not found at $memoryPath" -ForegroundColor Yellow
            }
        }
        "monitoring" {
...
function Start-ParallelEnvironment {
    ...
    foreach ($project in $projectsToStart) {
        Start-ParallelProject -ProjectName $project | Out-Null
