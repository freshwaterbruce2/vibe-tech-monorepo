...
# One-click launcher for memory sync system
# Usage: .\Start-MemorySync.ps1 [-Silent] [-RunOnStartup] [-ShowDashboard]
...
# Ensure D:\memory_bank exists
$memoryRoot = "D:\memory_bank"
if (-not (Test-Path $memoryRoot)) {
    New-Item -ItemType Directory -Path $memoryRoot -Force | Out-Null
    Write-Host "[OK] Created memory bank at $memoryRoot" -ForegroundColor Green
}

# Create subdirectories
@("projects", "prompts", "prompts\commands", "prompts\agents", ".hash_cache") | ForEach-Object {
    $path = Join-Path $memoryRoot $_
    if (-not (Test-Path $path)) {
        New-Item -ItemType Directory -Path $path -Force | Out-Null
    }
}

# Ensure D:\logs exists
...
$requiredPackages = @("watchdog", "pyyaml", "gitpython")
...
if __name__ == "__main__":
    main()
'@ | Out-File -FilePath $watcherScript -Encoding UTF8
    Write-Host "[OK] Created memory watcher script" -ForegroundColor Green
}

# Initialize git repo in memory bank if needed
if (-not (Test-Path "$memoryRoot\.git")) {
    Push-Location $memoryRoot
    git init --quiet
    git config user.name "Memory Sync"
    git config user.email "memory@local"
    git add .
    git commit -m "Initial memory bank setup" --quiet 2>$null
    Pop-Location
    Write-Host "[OK] Initialized git in memory bank" -ForegroundColor Green
}
...
if ($Silent) {
    ...
    Write-Host "[OK] Memory sync started in background" -ForegroundColor Green
    Write-Host "   View logs: Get-Content D:\logs\memory_sync.log -Tail 20" -ForegroundColor Gray
} elseif ($ShowDashboard) {
    Write-Host "[STARTING] Memory Sync with Dashboard..." -ForegroundColor Cyan
    Write-Host "   Config: $memoryRoot\memory_sync.yaml" -ForegroundColor Gray
    Write-Host "   Logs: D:\logs\memory_sync.log" -ForegroundColor Gray
