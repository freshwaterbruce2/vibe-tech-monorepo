...
export async function handleSpecificConstraintViolation(
	...
	if (
		!isPostgresError(error) ||
		error.constraint !== "IDX_1b101e71abe9ce72d910e95b9f"
	) {
		throw error instanceof Error ? error : new Error(String(error));
	}
  ...
  `;
	...
	try {
		const result = await db.query<ConstraintMetadata>(constraintQuery, [
		...
		if (result.rows.length > 0) {
			...
			console.warn(
				`Constraint violation on table: ${table_name}, column: ${column_name}`,
			);
