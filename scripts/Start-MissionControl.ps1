...
if (-not (Test-Path "V:\monorepo\CodingGuidelines.json")) {
...
$bridgeProcess = Start-Process -FilePath "npx" -ArgumentList "tsx V:\monorepo\backend\ipc-bridge\src\server.ts" -PassThru -NoNewWindow
...
$novaTrayApp = python V:\monorepo\apps\nova-agent\src\ui\tray_icon.py
$novaProcess = Start-Process -FilePath python -ArgumentList "V:\monorepo\apps\nova-agent\src\ui\tray_icon.py" -PassThru -NoNewWindow
...
$vibeProcess = Start-Process -FilePath "pnpm" -ArgumentList "--filter vibe-code-studio run electron:dev" -WorkingDirectory "V:\monorepo" -PassThru -NoNewWindow
