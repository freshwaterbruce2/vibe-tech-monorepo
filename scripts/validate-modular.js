...
function checkProjectStructure(projectPath) {
	...
	if (!hasModularStructure) {
		issues.push({
			...
			path: srcPath,
			suggestion:
				"Reorganize code into modules/, shared/, interfaces/, and config/ directories",
		});
	} else {
		console.log(
			`✅ ${projectName}: Found modular structure (${foundPatterns.join(", ")})`,
		);
	}
	...
	try {
		...
		for (const antiPattern of ANTI_PATTERNS) {
			if (fileName.toLowerCase().includes(antiPattern.toLowerCase())) {
				...
				if (antiPattern === "Manager" && fileName.includes("StateManager"))
					continue;
