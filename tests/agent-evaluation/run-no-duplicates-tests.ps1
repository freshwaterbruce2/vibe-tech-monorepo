...
if ($PSBoundParameters.ContainsKey('Verbose')) {
    ...
    Write-Host "pnpm tsx src/index.ts behavioral-eval behavioral-no-duplicates $Filter" -ForegroundColor DarkGray
}
...
try {
    ...
    if ($Filter) {
        $Output = pnpm tsx src/index.ts behavioral-eval behavioral-no-duplicates $Filter 2>&1
    } else {
        $Output = pnpm tsx src/index.ts behavioral-eval behavioral-no-duplicates 2>&1
    }
} finally {
    Set-Location $OldDir
}
