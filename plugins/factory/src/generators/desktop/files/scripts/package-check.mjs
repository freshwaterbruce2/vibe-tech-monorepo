...
await checkFile(path.join(projectRoot, 'dist', 'index.html'), 'frontend-dist', 'Frontend build output exists');
await checkJsonIncludes(path.join(tauriRoot, 'tauri.conf.json'), '"frontendDist"', 'tauri-config', 'Tauri config includes frontendDist wiring');
await checkJsonIncludes(path.join(tauriRoot, 'tauri.conf.json'), '"icons/icon.ico"', 'tauri-icons-config', 'Tauri config includes Windows icon wiring');
await checkFile(path.join(tauriRoot, 'Cargo.toml'), 'cargo-manifest', 'Cargo manifest exists');
await checkFile(path.join(tauriRoot, 'build.rs'), 'tauri-build-script', 'Tauri build script exists');
await checkFile(path.join(tauriRoot, 'icons', 'icon.ico'), 'tauri-windows-icon', 'Windows icon exists');
...
async function checkMsvcRuntime() {
  ...
  checks.push({
    id: 'msvc-runtime',
    ok: false,
    detail: 'No desktop x64 msvcrt.lib was found in LIB, VCToolsInstallDir, or installed Visual Studio roots',
  });
}
...
async function findMsvcRuntime() {
  ...
  for (const candidate of unique(desktopCandidates).filter((candidatePath) => !isOneCorePath(candidatePath))) {
