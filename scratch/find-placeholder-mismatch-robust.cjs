...
function analyzeFile(file) {
  ...
  // Find all matches for prepare(...)
  // We scan char by char to handle template strings correctly.
  let i = 0;
  while (i < content.length) {
    if (content.slice(i, i + 8) === 'prepare(') {
      const startIndex = i;
      i += 8;
      // Parse until the matching closing parenthesis of prepare(
      let parenCount = 1;
      ...
      // Find execution call: search for the next execution (.all, .run, .get)
      let execIndex = -1;
      let execType = '';
      const searchWindow = content.slice(i, i + 2000);
      
      // Let's search char by char in the search window for .all(, .run(, .get(
      let j = 0;
      ...
      if (execIndex !== -1) {
        ...
        // Parse args until matching closing parenthesis of execution call
...
const allFiles = [
  'C:\\dev\\packages\\memory\\src\\stores\\EpisodicStore.ts',
  'C:\\dev\\packages\\memory\\src\\stores\\SemanticStore.ts',
  'C:\\dev\\packages\\memory\\src\\cognitive\\CognitiveMemoryAdapter.ts',
  'C:\\dev\\packages\\memory\\src\\consolidation\\MemoryDecay.ts',
  'C:\\dev\\packages\\memory\\src\\integrations\\LearningBridge.ts',
];
