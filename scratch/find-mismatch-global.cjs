  ...
  try {
    ...
    list.forEach((file) => {
      ...
      if (stat && stat.isDirectory()) {
        ...
        if (bn !== 'node_modules' && bn !== 'dist' && bn !== '.git' && bn !== '.claude' && bn !== '_worktrees' && bn !== '.nx' && bn !== 'coverage') {
    ...
    // Ignore permissions/missing folders
  }
  return results;
}
...
console.log('Finding all code files...');
const files = getFiles('C:\\dev');
