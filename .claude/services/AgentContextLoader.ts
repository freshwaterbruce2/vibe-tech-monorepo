...
export class AgentContextLoader {
  ...
  async loadContext(options: LoadContextOptions): Promise<LoadedContext> {
    ...
    if (level >= 3) {
      ...
      if (includeLearning) {
        result.learning = await this.loadLearningPatterns(agentName, projectName);
  ...
  private loadProjectContext(projectName: string): string {
    ...
    if (!existsSync(projectPath)) {
      console.warn(`Project context not found: ${projectPath}, using empty context`);
      return '';
    }
  ...
  public getCacheStats() {
    ...
    return {
      ...
      avgAge: entries.length
        ? entries.reduce((sum, [_, e]) => sum + (now - e.timestamp), 0) / entries.length / 1000
        : 0,
