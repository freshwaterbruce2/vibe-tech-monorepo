...
function Get-ProjectContext {
    ...
    # Handle apps structure: apps/project-name (most common)
    if ($NormalizedPath -match 'apps/([^/]+)(?:/|$)') {
        return $matches[1]
    }

    # Handle packages structure: packages/package-name
    if ($NormalizedPath -match 'packages/([^/]+)(?:/|$)') {
        return $matches[1]
    }

    # Handle backend structure: backend/service-name
    if ($NormalizedPath -match 'backend/([^/]+)(?:/|$)') {
        return $matches[1]
    }
...
# ============================================================================
# Planning-with-Files Integration (Phase 1.5)
# ============================================================================
...
function Get-PlanningThresholds {
    try {
        if (Test-Path $PlanningThresholdsFile) {
            return Get-Content $PlanningThresholdsFile -Raw | ConvertFrom-Json
        }
    } catch {}
    # Default thresholds if config not found
    return @{
        thresholds = @{
            fileCount = @{ planningRequired = 3 }
            linesChanged = @{ planningRequired = 500 }
        }
        complexityTriggers = @{
            automatic = @(
                @{ trigger = "NEW_FEATURE"; requiresPlanning = $true }
                @{ trigger = "BREAKING_CHANGE"; requiresPlanning = $true }
                @{ trigger = "DATABASE_CHANGE"; requiresPlanning = $true }
                @{ trigger = "REFACTOR_MAJOR"; requiresPlanning = $true }
            )
        }
    }
}

function Should-SuggestPlanning {
    param([string]$Prompt, $Analysis, $SystemContext)

    $Thresholds = Get-PlanningThresholds
    $Lower = $Prompt.ToLower()

    # Check complexity triggers from prompt text
    $TriggerKeywords = @{
        "NEW_FEATURE" = @('implement', 'create new', 'add feature', 'build', 'develop')
        "BREAKING_CHANGE" = @('breaking', 'migrate', 'upgrade major', 'deprecate')
        "DATABASE_CHANGE" = @('database', 'schema', 'migration', 'table', 'column')
        "REFACTOR_MAJOR" = @('refactor', 'restructure', 'redesign', 'rewrite', 'overhaul')
        "CROSS_MODULE" = @('across', 'multiple', 'all apps', 'monorepo-wide', 'workspace')
        "SECURITY_CHANGE" = @('security', 'authentication', 'authorization', 'encrypt', 'token')
        "ARCHITECTURE_CHANGE" = @('architecture', 'pattern', 'service', 'microservice')
    }

    foreach ($trigger in $TriggerKeywords.Keys) {
        foreach ($keyword in $TriggerKeywords[$trigger]) {
            if ($Lower -match [regex]::Escape($keyword)) {
                return @{
                    should_suggest = $true
                    reason = "Task involves $trigger"
                    trigger = $trigger
                }
            }
        }
    ...
    if ($SystemContext.modified_count -ge $Thresholds.thresholds.fileCount.planningRequired) {
        return @{
            should_suggest = $true
            reason = "Currently modifying $($SystemContext.modified_count) files (threshold: $($Thresholds.thresholds.fileCount.planningRequired))"
            trigger = "FILE_COUNT"
        }
    }

    # Check complexity score
    if ($Analysis.complexity -ge 5) {
        return @{
            should_suggest = $true
            reason = "High complexity task (score: $($Analysis.complexity))"
...
function Get-ActivePlanningSession {
    param([string]$Project = "workspace")

    if (-not (Test-Path $PlanningFilesPath)) {
        return $null
    }

    try {
        # Find most recent session directory matching pattern YYYYMMDD-HHMMSS
        $SessionDirs = Get-ChildItem -Path $PlanningFilesPath -Directory -Recurse -Depth 2 |
            Where-Object { $_.Name -match '^\d{8}-\d{6}$' } |
            Sort-Object LastWriteTime -Descending |
            Select-Object -First 5

        foreach ($dir in $SessionDirs) {
            $TaskPlanPath = Join-Path $dir.FullName "task_plan.md"
            if (Test-Path $TaskPlanPath) {
                $Content = Get-Content $TaskPlanPath -Raw
                # Check if session is still in progress
                if ($Content -match 'Status:\s*in_progress') {
                    # Check if it matches the project
                    if ($Content -match "Project:\s*$Project" -or $Content -match "Project:\s*workspace") {
                        return @{
                            session_id = $dir.Name
                            path = $dir.FullName
                            project = $Project
...
function Suggest-PlanningWorkflow {
    param($SuggestionResult, $ActiveSession, [string]$Project)

    if (-not $SuggestionResult.should_suggest) {
        return
    }

    Write-Host ""
    Write-Host "PLANNING SUGGESTION" -ForegroundColor Cyan
    Write-Host "===================" -ForegroundColor Cyan

    if ($ActiveSession) {
        Write-Host "  Active Session Found: $($ActiveSession.session_id)" -ForegroundColor Green
        Write-Host "  Path: $($ActiveSession.path)" -ForegroundColor Gray
        Write-Host ""
        Write-Host "  Run '/planning-with-files:recover' to restore context" -ForegroundColor Yellow
    } else {
        Write-Host "  Reason: $($SuggestionResult.reason)" -ForegroundColor Yellow
        Write-Host "  Trigger: $($SuggestionResult.trigger)" -ForegroundColor Gray
        Write-Host ""
        Write-Host "  Consider starting a planning session:" -ForegroundColor White
        Write-Host "    /planning-with-files:start" -ForegroundColor Green
        Write-Host ""
        Write-Host "  Benefits:" -ForegroundColor White
        Write-Host "    - Persistent context across sessions" -ForegroundColor Gray
        Write-Host "    - Track progress with task_plan.md" -ForegroundColor Gray
        Write-Host "    - Store findings in findings.md" -ForegroundColor Gray
        Write-Host "    - Easy recovery with /planning-with-files:recover" -ForegroundColor Gray
    }

    Write-Host ""
    Write-Host "  (This suggestion is opt-in. Continue without planning if preferred.)" -ForegroundColor DarkGray
    Write-Host ""
}
